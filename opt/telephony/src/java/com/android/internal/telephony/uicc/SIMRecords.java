    ...
    public boolean isHPlmn(String plmn){
        ...
        if(mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_GSM){
            ...
            if((plmn != null) && mccmnc == null){
                for(int i=0; i <GsmServiceStateTracker.customEhplmn.length; i++){
                    //reset flag
                    isServingPlmnInGroup = false;
                    isHomePlmnInGroup = false;

                    //check if target plmn or home plmn in this group
                    for(int j=0; j<	GsmServiceStateTracker.customEhplmn[i].length;j++){
                        if(plmn.equals(GsmServiceStateTracker.customEhplmn[i][j])){
                            isServingPlmnInGroup = true;
                        }
                        if(mccmnc.equals(GsmServiceStateTracker.customEhplmn[i][j])){
