...
status_t AudioFlinger::SetStereoToMonoFlag(int device)
{
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
    if (audio_is_linear_pcm(format) && (format != AUDIO_FORMAT_PCM_16_BIT) && (format != AUDIO_FORMAT_PCM_8_24_BIT) && (format != AUDIO_FORMAT_PCM_32_BIT) ) {
#else
...

    {
        ...
        if (sessionId != NULL && *sessionId != AUDIO_SESSION_OUTPUT_MIX) {
            ...
            if ((streamType == AUDIO_STREAM_MUSIC) && (strcmp(lpaEnabled, "on") == 0)) {
                ...
                ds1SendBroadcastMessage(String16("LPA_SESSION_ID_CHANGED_ACTION"), *sessionId);
                ...
                if (lastLpaSessionId != LPA_INVALID_SESSION_ID) {
                    ds1SendBroadcastMessage(String16("LPA_SESSION_ID_REMOVED_ACTION"), lastLpaSessionId);
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
            ( (config.format != AUDIO_FORMAT_PCM_16_BIT) && (config.format != AUDIO_FORMAT_PCM_32_BIT) ) ||
#else
