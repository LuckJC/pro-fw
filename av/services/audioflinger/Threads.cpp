...
#ifdef MTK_AUDIO_DCREMOVAL
        ,mDcRemove(NULL),mDcRemoveWorkBuffer(NULL),DcRemoveBufferSize(0)
#endif
...
#endif
{
    ...
    #else
    mDcRemoveWorkBuffer = new int16_t[mNormalFrameCount * mChannelCount];
    memset(mDcRemoveWorkBuffer, 0, mNormalFrameCount * mChannelCount * sizeof(int16_t));
    #endif
    #endif
}
...
void AudioFlinger::PlaybackThread::readOutputParameters()
{
    ...
    mAllocMixBuffer = new int8_t[mNormalFrameCount * mFrameSize * 2 + align - 1];
    mMixBuffer = (int32_t *) ((((size_t)mAllocMixBuffer + align - 1) / align) * align);
    memset(mMixBuffer, 0, mNormalFrameCount * mFrameSize * 2);
...
if (mType != DIRECT)
#ifdef MTK_HD_AUDIO_ARCHITECTURE
    POSTPRO_PATCH_JB_OUTPROC_PLAY_SAMPLES(this, mFormat, mMixBuffer, mNormalFrameCount* mChannelCount*sizeof(int32_t), mSampleRate, mChannelCount);
#else
...
#define mBitShift 2 // FIXME
        ...
        size_t offset = (mCurrentWriteLength - mBytesRemaining) >> (mBitShift-1);
        ATRACE_BEGIN("write");
        ...
        ssize_t framesWritten;
         MTK_ALOG_D("has fast mixer %d, count %d",hasFastMixer(), count );
        if (mFormat == AUDIO_FORMAT_PCM_32_BIT) {
            framesWritten = mNormalSink->write(mMixBuffer + offset, count);
        ...
        const sp<AudioFlinger::PlaybackThread::Track> track, int EffectId)
{
    ...
    ALOGV("PlayckThread::attachAuxEffect_l:effectid:%d",EffectId);
    if (EffectId == 0) {
        ...
        if (effect != 0) {
            if ((effect->desc().flags & EFFECT_FLAG_TYPE_MASK) == EFFECT_FLAG_TYPE_AUXILIARY) {
                ALOGV("PlayckThread::attachAuxEffect_l:set aux buf");
                track->setAuxBuffer(EffectId, (int32_t *)effect->inBuffer());
            } else {
                ALOGV("PlayckThread::attachAuxEffect_l,not aux effect");
                status = INVALID_OPERATION;
            }
        } else {
            ALOGV("PlayckThread::attachAuxEffect_l,effect==0");
            status = BAD_VALUE;
        }
    }
    return status;
        ...
        // mNormalSink below
{
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
    ALOGD("offers stream format %d",Format_from_SR_C(mSampleRate, mChannelCount, output->stream->common.get_format(&(output->stream->common))  ));
    const NBAIO_Format offers[1] = {Format_from_SR_C(mSampleRate, mChannelCount, output->stream->common.get_format(&(output->stream->common))  )};
#else
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
        fastTrack->mSampleRate = 44100;
        fastTrack->mFormat = AUDIO_FORMAT_PCM_32_BIT;
#endif
    ...
    case FastMixer_Static:
        ...
        if(mDcRemove)
        {
            mDcRemove->init(mChannelCount, mSampleRate, DcRemove::DCR_MODE_3);        
        }
#endif
}
...
void AudioFlinger::MixerThread::threadLoop_mix()
{
    ...
    if(mDcRemove)
    {
        if(DcRemoveBufferSize!=mixBufferSize && mixBufferSize)
        {
            //if(mDcRemoveWorkBuffer)
            //    delete[] mDcRemoveWorkBuffer;    
            ...
            #else
            DcRemoveBufferSize = mNormalFrameCount*mChannelCount *sizeof(int16_t);
            #endif
        }

        if(DcRemoveBufferSize)
        {           
            //size_t outputSize = mDcRemove->process(mMixBuffer, mixBufferSize, mDcRemoveWorkBuffer);
            size_t outputSize = mDcRemove->process(mMixBuffer, DcRemoveBufferSize, mDcRemoveWorkBuffer);
            //memcpy((int8_t *)mMixBuffer,(int8_t *)mDcRemoveWorkBuffer,DcRemoveBufferSize);
            memcpy((int8_t *)mMixBuffer,(int8_t *)mDcRemoveWorkBuffer,outputSize);
        }
    } 
#ifdef DEBUG_AUDIO_PCM
        #ifdef MTK_HD_AUDIO_ARCHITECTURE
        AudioDump::dump(gaf_mixer_pcm,mMixBuffer,mNormalFrameCount*mChannelCount *sizeof(int32_t),gaf_mixer_propty);
        #else
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
        memset(mMixBuffer, 0, mNormalFrameCount * mChannelCount * sizeof(int32_t));
#else
    ...
    for (size_t i=0 ; i<count ; i++) {
        ...
        if (track->isFastTrack()) {
            ...
            if (isActive) {
                ...
                if (!(state->mTrackMask & (1 << j))) {
                    ...
                    ALOGD("fastTrack->mFormat %d, track format %d",fastTrack->mFormat,track->format());
#endif
                ...
                !track->isPaused() && !track->isTerminated())
        {
            ...
            mAudioMixer->setParameter(
                name,
                AudioMTKMixer::DRC,
                AudioMTKMixer::STEREO2MONO, (void *)mAudioFlinger->mSteroToMono);
            mAudioMixer->setParameter(
                name,
                AudioMTKMixer::DRC,
...
#endif
#ifdef MTK_HD_AUDIO_ARCHITECTURE
            // always do DC removal for normal track
            mAudioMixer->setParameter(
                        name,
                        AudioMTKMixer::TRACK,
                        AudioMTKMixer::DO_POST_PROC, (void *) 1);
#endif
...
int AudioFlinger::MixerThread::getTrackName_l(audio_channel_mask_t channelMask, audio_format_t format, int sessionId)
{
    ...
    while (!mNewParameters.isEmpty()) {
        ...
        if (status == NO_ERROR) {
            ...
            if (status == NO_ERROR && reconfig) {
                ...
                for (size_t i = 0; i < mTracks.size() ; i++) {
#ifdef MTK_HD_AUDIO_ARCHITECTURE
                    int name = getTrackName_l(mTracks[i]->mChannelMask, mTracks[i]->mFormat, mTracks[i]->mSessionId);
#else
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
int AudioFlinger::DirectOutputThread::getTrackName_l(audio_channel_mask_t channelMask, audio_format_t format, int sessionId)
{
    return 0;
}
#else
// getTrackName_l() must be called with ThreadBase::mLock held
int AudioFlinger::DirectOutputThread::getTrackName_l(audio_channel_mask_t channelMask, int sessionId)
{
    return 0;
}
#endif
        ...
        if (mActiveTrack != 0) {
            ...
            if (status == NO_ERROR) {
                ...
                if (mFramestoDrop == 0) {
                   ...
                   AudioDump::dump(gaf_record_pcm, buffer.raw, buffer.frameCount*mActiveTrack->mFrameSize,gaf_record_propty);
#endif
                    mActiveTrack->releaseBuffer(&buffer);
                } else {
                    if (mFramestoDrop > 0) {
