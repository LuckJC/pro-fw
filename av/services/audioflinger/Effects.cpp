...
status_t AudioFlinger::EffectChain::addEffect_l(const sp<EffectModule>& effect)
{
    ...
    ALOGD("call effect configure,%s",effect->desc().name);
    ...
    if (suspend) {
        if (index >= 0) {
            ...
            if (memcmp(&desc->mType, EFFECT_SL_IID_DS, sizeof(effect_uuid_t)) == 0)
                ds1SendBroadcastMessage(String16("DS_EFFECT_SUSPEND_ACTION"), 1);
#endif // DOLBY_END
#endif
        }
        if (desc->mRefCount++ == 0) {
...
#ifdef DOLBY_DAP_OPENSLES
            if (memcmp(&desc->mType, EFFECT_SL_IID_DS, sizeof(effect_uuid_t)) == 0)
                ds1SendBroadcastMessage(String16("DS_EFFECT_SUSPEND_ACTION"), 0);
#endif // DOLBY_END
#endif
