...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s][msg='%s' ][what=kWhatComponentAllocated]:IN",__FUNCTION__,msg->debugString().c_str());
#endif
                ...
                {
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s][msg='%s' ][what=kWhatComponentConfigured]:IN",__FUNCTION__,msg->debugString().c_str());
#endif
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s][msg='%s' ][what=kWhatInputSurfaceCreated]:IN",__FUNCTION__,msg->debugString().c_str());
#endif
                ...
                {
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s][msg='%s' ][what=kWhatSignaledInputEOS]:IN",__FUNCTION__,msg->debugString().c_str());
#endif
                ...
                {
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s][msg='%s' ][what=kWhatBuffersAllocated]:IN",__FUNCTION__,msg->debugString().c_str());
#endif
                    int32_t portIndex;
                ...
                {
                    ALOGI("codec output format changed mFlags %x", mFlags);
                ...
                {
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s][msg='%s' ][what=kWhatShutdownCompleted][mState=%d]:IN",
		__FUNCTION__,msg->debugString().c_str(),mState);
#endif
                    if (mState == STOPPING) {
                        setState(INITIALIZED);
                    } else {
...
#ifndef ANDROID_DEFAULT_CODE
            AString mimeTmp;
            format->findString("mime", &mimeTmp);
            if (!strncasecmp("video/", mimeTmp.c_str(), 6))
            {
                if ( (obj == NULL) && (flags != CONFIGURE_FLAG_ENCODE) ) {
                    format->setInt32("outputNormalYUV", true);
                    ALOGD("offer NormalYUV for JAVA framework enabled");
                }
            }
#endif //ANDROID_DEFAULT_CODE colorconvert
            extractCSD(format);
#ifndef ANDROID_DEFAULT_CODE
		if(mCrypto != NULL){
			ALOGI("[test]set IsSecure ");
...
void MediaCodec::extractCSD(const sp<AMessage> &format) {
    ...
    ALOGI("Found %u pieces of codec specific data.", mCSD.size());
}
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] csd->size()  %d  > codecInputData->capacity() %d",__FUNCTION__ ,csd->size() , codecInputData->capacity());
#endif
        return -EINVAL;
    }
...
status_t MediaCodec::onQueueInputBuffer(const sp<AMessage> &msg) {
    ...
    if (msg->findSize("size", &size)) {
        ...
        if (mCrypto == NULL) {
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] mCrypto == NULL",__FUNCTION__ );
#endif
            return -EINVAL;
        }
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] index %d > mPortBuffers[%d].size() %d",
		__FUNCTION__,index , kPortIndexInput,mPortBuffers[kPortIndexInput].size());
#endif
        return -ERANGE;
    }
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] %d,%d",__FUNCTION__,(info->mNotify == NULL),(!info->mOwnedByClient));
#endif
        return -EACCES;
    }
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] offset=%d , size %d >info->mEncryptedData->capacity() %d  ",__FUNCTION__,offset,size,info->mEncryptedData->capacity());
#endif
        return -EINVAL;
    }
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] size %d >info->mEncryptedData->capacity() %d  ",__FUNCTION__,size,info->mEncryptedData->capacity());
#endif
            return -ERANGE;
        }
...
status_t MediaCodec::onReleaseOutputBuffer(const sp<AMessage> &msg) {
    ...
    if (mState != STARTED) {
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] mState != STARTED",__FUNCTION__);
#endif
        return -EINVAL;
    }
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGI("[%s] index >size  ",__FUNCTION__);
#endif
        return -ERANGE;
    }
...
#ifndef ANDROID_DEFAULT_CODE
	ALOGV("[%s]portIndex=%d no buffer avail",__FUNCTION__,portIndex);
#endif
        return -EAGAIN;
    }
