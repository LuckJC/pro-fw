    ...
    if (mNativeWindow != NULL && portIndex == kPortIndexOutput) {
        ...
        if (err == OK) {
            ...
            if((portIndex == kPortIndexOutput) && (mIsVideoDecoder) && (mIsDemandNormalYUV==true))                
            {
                ...
                totalSize = def.nBufferCountActual * (mAlignedSize);
            }
            else
        ...
        if (mIsVideoDecoder || mIsVideoEncoder) {
            ...
            for (OMX_U32 i = 0; i < def.nBufferCountActual; ++i) {
                ...
                if((portIndex == kPortIndexOutput) && (!strncmp("OMX.MTK.VIDEO.DECODER.", mComponentName.c_str(), strlen("OMX.MTK.VIDEO.DECODER."))) && (mIsDemandNormalYUV==true))
                {
                    mem = mDealer[portIndex]->allocate((mAlignedSize));
                }
                else
                {
...
#else
                    memforcolorconvert = mDealerForColorConvert[kPortIndexOutputForColorConvert]->allocate((mAlignedSize));
                ...
                if((portIndex == kPortIndexOutput) && (!strncmp("OMX.MTK.VIDEO.DECODER.", mComponentName.c_str(), strlen("OMX.MTK.VIDEO.DECODER."))) && (mIsDemandNormalYUV==true))
                {
                    ...
                    if (mem != NULL) 
                    {
#ifdef USE_COLOR_CONVERT_MVA
                        {
                            ...
                            info.mData = new ABuffer(mem->pointer(), (mAlignedSize));
                    ...
                    if (memforcolorconvert != NULL) {
                        ...
                        OMX_U32 pBuffer = ((reinterpret_cast<OMX_U32>(ptr)+(MTK_BUF_ADDR_ALIGNMENT_VALUE-1))&~(MTK_BUF_ADDR_ALIGNMENT_VALUE-1));
                        infoForOMX.mData = new ABuffer((void*)pBuffer, (mAlignedSize));
...
status_t ACodec::freeBuffersOnPort(OMX_U32 portIndex) {
    ...
    else if((portIndex == kPortIndexOutput) && (!strncmp("OMX.MTK.VIDEO.DECODER.", mComponentName.c_str(), strlen("OMX.MTK.VIDEO.DECODER."))) && (mIsDemandNormalYUV==true))
    {
#ifdef USE_VIDEO_ION
        for (int i = 0 ; i < MAX_ION_BUF_COUNT ; i++) {
            if (mIonOutputBufInfo[i].IonFd  != 0xFFFFFFFF) {
    ...
    switch (type) {
        ...
        case omx_message::FILL_BUFFER_DONE:
        {
            ...
            if( (!strncmp("OMX.MTK.VIDEO.DECODER.", mCodec->mComponentName.c_str(), strlen("OMX.MTK.VIDEO.DECODER."))) && (mCodec->mIsDemandNormalYUV==true))
            {
                ...
                ALOGV("Tid %d, mM4UMVABufferPa %p, mM4UVABufferVa %p", androidGetTid(), mplatform_privateK.mM4UMVABufferPa, mplatform_privateK.mM4UVABufferVa);
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            if (buffer != NULL && !mCodec->mPortEOS[kPortIndexInput]) {
                ...
                ALOGD("[%s] calling emptyBuffer %p signalling EOS",
                     mCodec->mComponentName.c_str(), bufferID);
...
bool ACodec::BaseState::onOMXFillBufferDone(
    ...
    if( (!strncmp("OMX.MTK.VIDEO.DECODER.", mCodec->mComponentName.c_str(), strlen("OMX.MTK.VIDEO.DECODER."))) && (mCodec->mIsDemandNormalYUV==true))
    {
        ...
        CHECK_EQ(mCodec->mOMX->getParameter(
                 ...
                 (status_t)OK);
        OMX_VIDEO_PORTDEFINITIONTYPE *videoDef = &def.format.video;
        LOGV("nFrameWidth %d, nFrameHeight %d, nStride %d, nSliceHeight %d, src %p, dts %p, outputportPtr(MVA) %p", videoDef->nFrameWidth, 
        videoDef->nFrameHeight, videoDef->nStride, videoDef->nSliceHeight, infoOmx->mData->data(), info->mData->data(), outputportPtr);
        ...
        //if (!strncmp("OMX.MTK.", mComponentName.c_str(), 8)) 
        {
            uint32_t eHalColorFormat = HAL_PIXEL_FORMAT_I420;//tmp for build pass
            switch (def.format.video.eColorFormat) {
                case OMX_COLOR_FormatYUV420Planar:
                    eHalColorFormat = HAL_PIXEL_FORMAT_I420;
            ...
            ALOGV ("mSrcFormat(0x%x), eHalColorFormat(0x%x)", mSrcFormat, eHalColorFormat);
        }
...
#ifndef ANDROID_DEFAULT_CODE
        ALOGD("native_window_set_crop l(%d), t(%d), r(%d), b(%d)", crop.left, crop.top, crop.right, crop.bottom);
#endif //ANDROID_DEFAULT_CODE
...
#ifndef ANDROID_DEFAULT_CODE
                    if( info != NULL )
                        ALOGD("dequeueBufferFromNativeWindow %p", info->mBufferID);
#endif //ANDROID_DEFAULT_CODE
                }
...
int ACodec::BaseState::Mtk_ACodecConvertDecoderOutputToI420(
    ...
    {
        mRetDstSize = YUV_SIZE(srcWidth, srcHeight);
    }
        ...
        ALOGV("convert t: = %lld during %lld ~ %lld, avg %lld micro sec", diff, mCodec->mProfileColorConvout_timeMin, mCodec->mProfileColorConvout_timeMax, 
