...
#ifndef ANDROID_DEFAULT_CODE
static int64_t kHttpHighWaterMarkUs = 10000000ll;  // only for http 10secs, widevine would use 5s, if else ,widevine would fai ALPS01441458
#endif
...
status_t AwesomePlayer::finishSetDataSource_l() {
	...
	if (removeSpecificHeaders(String8("MTK-HTTP-CACHE-SIZE"), &mUriHeaders, &cacheSize)) {
       ...
       ALOGD("http cache size(us) = %lld", mHighWaterMarkUs);
#endif
        String8 cacheConfig;
