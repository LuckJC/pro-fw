...
public class PhoneWindowManager implements WindowManagerPolicy {
	...
	private static final String KEY_GESTURE_SMS_VALUE = "gesture_sms_value";
	private static final String KEY_GESTURE_WAKEUP_VALUE = "gesture_wakeup_value";
	private static final String KEY_GESTURE_MUSIC_VALUE = "gesture_music_value";
	private static final String KEY_GESTURE_CAMERA_VALUE = "gesture_camera_value";
	private static final String KEY_GESTURE_BROWSER_VALUE = "gesture_browser_value";
	private static final String KEY_GESTURE_CALL_VALUE = "gesture_call_value";
	
	/**
	 * These are the system UI flags that, when changing, can cause the layout
	 * of the screen to change.
	 */
	static final int SYSTEM_UI_CHANGING_LAYOUT = View.SYSTEM_UI_FLAG_HIDE_NAVIGATION
	 ...
	 * applications, such as a web browser. HID defines nearly a hundred of them
	 * in the Consumer (0x0C) usage page. We don't support quite that many
	 * yet...
	 */
	...
	static {
		...
		sApplicationLaunchKeyCategories.append(KeyEvent.KEYCODE_SETTINGS, "android.intent.category.APP_SETTINGS");
		sApplicationLaunchKeyCategories.append(KeyEvent.KEYCODE_MESSAGING, "android.intent.category.APP_MESSAGING");
		sApplicationLaunchKeyCategories.append(KeyEvent.KEYCODE_GALLERY, Intent.CATEGORY_APP_GALLERY);
		// / @}
	}
	...
	boolean mNavigationBarCanMove = false; // can the navigation bar ever move
											// to the side?
	boolean mNavigationBarOnBottom = true; // is the navigation bar on the
											// bottom *right now*?
	...
	// Default display does not rotate, apps that require non-default
	// orientation will have to
	...
	// the screen and doesn't account for any system elements like the status
	// bar.
	int mOverscanScreenLeft, mOverscanScreenTop;
	int mOverscanScreenWidth, mOverscanScreenHeight;
	// The current visible size of the screen; really; (ir)regardless of whether
	// the status
	...
	// fullscreen window flag, these are the stable dimensions without the
	// status bar.
	...
	private static final int DISMISS_KEYGUARD_NONE = 0; // Keyguard not being
														// dismissed.
	private static final int DISMISS_KEYGUARD_START = 1; // Keyguard needs to be
															// dismissed.
	private static final int DISMISS_KEYGUARD_CONTINUE = 2; // Keyguard has been
															// dismissed.
	int mDismissKeyguard = DISMISS_KEYGUARD_NONE;

	/**
	 * The window that is currently dismissing the keyguard. Dismissing the
	 * keyguard must only be done once per window.
	 */
	...
	// / M: [ALPS00062902]The global variable to save the state of
	// stk.enable.user_activity
	boolean mIsStkUserActivityEnabled = false;
	// / M: [ALPS00062902]Protect mIsStkUserActivityEnabled be accessed at the
	// multiple places
	private Object mStkLock = new Object();
	// / M:Screen on debugging mechanism @{
	MessageMonitorLogger mMessageLogger = null;
	static final int SCREEN_ON_MONITOR_TIMEOUT = 5000;
	// / @}
	...
	private UEventObserver mHDMIObserver = new UEventObserver() {
		...
		public void onUEvent(UEventObserver.UEvent event) {
			if (FeatureOption.MTK_SMARTBOOK_SUPPORT && mHDMI != null) {
				int type = mHDMI.getDisplayType();
				Slog.d(TAG, "onUEvent, HDMI type:" + type + " event:" + event);
				if (1 == type || mSmartBookPlugIn) { // SmartBook HDMI plug
					try {
						mWindowManager.notifySmartBookState("1".equals(event
								.get("SWITCH_STATE")));
					} catch (RemoteException e) {
						Slog.e(TAG, "notifySmartBookState error:", e);
					}
				}
			}
	...
	private final BarController mStatusBarController = new BarController("StatusBar", View.STATUS_BAR_TRANSIENT, View.STATUS_BAR_UNHIDE,
												View.STATUS_BAR_TRANSLUCENT, StatusBarManager.WINDOW_STATUS_BAR,
												WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);

	private final BarController mNavigationBarController = new BarController("NavigationBar", View.NAVIGATION_BAR_TRANSIENT,
												View.NAVIGATION_BAR_UNHIDE, View.NAVIGATION_BAR_TRANSLUCENT,
	...
	void updateOrientationListenerLp() {
		...
		if (localLOGV)
			Slog.v(TAG, "Screen status=" + mScreenOnEarly
	...
	private void interceptScreenshotChord() {
		if (mScreenshotChordEnabled && mVolumeDownKeyTriggered
	...
	private long getScreenshotChordLongPressDelay() {
		if (mKeyguardDelegate.isShowing()) {
			...
			return (long) (KEYGUARD_SCREENSHOT_CHORD_DELAY_MULTIPLIER * ViewConfiguration
	...
	private final Runnable mPowerLongPress = new Runnable() {
		...
		public void run() {
			...
			case LONG_PRESS_POWER_GLOBAL_ACTIONS:
				...
				if (!performHapticFeedbackLw(null,
	...
	public void init(Context context, IWindowManager windowManager,	WindowManagerFuncs windowManagerFuncs) {
		...
		try {
			mOrientationListener
					.setCurrentRotation(windowManager.getRotation());
		...
		// / M:[AppLaunchTime] Improve the mechanism of AppLaunchTime
		mAppLaunchTimeEnabled = (1 == SystemProperties.getInt(
				"persist.applaunchtime.enable", 0)) ? true : false;
		...
		MessageMonitorLogger.MessageLoggerCallbacks callback = new MessageMonitorLogger.MessageLoggerCallbacks() {
			...
			public void onMessageTimeout(Message msg) {
				Log.d("ScreenOnMonitor","Monitor message timeout, start to dump all messages in system process");
				mMessageLogger.dumpAllMessageHistory();
		...
		if (FeatureOption.MTK_SMARTBOOK_SUPPORT) {
			try {
				...
				Slog.e(TAG, "Create instance error: ", e);
			}
		}
		// / @}
	}
	...
	/**
	 * Read values from config.xml that may be overridden depending on the
	 * configuration of the device. eg. Disable long press on home goes to
	 * recents on sw600dp.
	 */
	private void readConfigurationDependentBehaviors() {
		...
		if (mLongPressOnHomeBehavior < LONG_PRESS_HOME_NOTHING || mLongPressOnHomeBehavior > LONG_PRESS_HOME_ASSIST) {
	...
	public void setInitialDisplaySize(Display display, int width, int height,
			int density) {
		// This method might be called before the policy has been fully
		// initialized
		...
		mNavigationBarHeightForRotation[mPortraitRotation] = mNavigationBarHeightForRotation[mUpsideDownRotation] = res
				.getDimensionPixelSize(com.android.internal.R.dimen.navigation_bar_height);
		mNavigationBarHeightForRotation[mLandscapeRotation] = mNavigationBarHeightForRotation[mSeascapeRotation] = res
				.getDimensionPixelSize(com.android.internal.R.dimen.navigation_bar_height_landscape);

		// Width of the navigation bar when presented vertically along one side
		mNavigationBarWidthForRotation[mPortraitRotation] = mNavigationBarWidthForRotation[mUpsideDownRotation] = mNavigationBarWidthForRotation[mLandscapeRotation] = mNavigationBarWidthForRotation[mSeascapeRotation] = res
		...
		mDemoHdmiRotationLock = SystemProperties.getBoolean(
				"persist.demo.hdmirotationlock", false);

		// Only force the default orientation if the screen is xlarge, at least
		// 960dp x 720dp, per
		// http://developer.android.com/guide/practices/screens_support.html#range
		mForceDefaultOrientation = longSizeDp >= 960
				&& shortSizeDp >= 720
				...
				!"true".equals(SystemProperties
						.get("config.override_forced_orient"));
	}
	...
	@Override
	public void setDisplayOverscan(Display display, int left, int top,
	...
	public void updateSettings() {
		...
		synchronized (mLock) {
			...
			int userRotation = Settings.System.getIntForUser(resolver,
					Settings.System.USER_ROTATION, Surface.ROTATION_0, UserHandle.USER_CURRENT);
			if (mUserRotation != userRotation) {
				mUserRotation = userRotation;
				updateRotation = true;
			}
			int userRotationMode = Settings.System.getIntForUser(resolver, Settings.System.ACCELEROMETER_ROTATION, 0,
					UserHandle.USER_CURRENT) != 0 ? WindowManagerPolicy.USER_ROTATION_FREE : WindowManagerPolicy.USER_ROTATION_LOCKED;
	...
	private void disablePointerLocation() {
		if (mPointerLocationPointerEventListener != null) {
			mWindowManagerFuncs.unregisterPointerEventListener(mPointerLocationPointerEventListener);
	...
	public boolean checkShowToOwnerOnly(WindowManager.LayoutParams attrs) {
		...
		// If this switch statement is modified, modify the comment in the
		// declarations of
		// the type in {@link WindowManager.LayoutParams} as well.
		switch (attrs.type) {
		default:
			// These are the windows that by default are shown only to the user
			// that created
			// them. If this needs to be overridden, set
			// {@link
			// WindowManager.LayoutParams.PRIVATE_FLAG_SHOW_FOR_ALL_USERS} in
			// {@link WindowManager.LayoutParams}. Note that permission
			// {@link android.Manifest.permission.INTERNAL_SYSTEM_WINDOW} is
			// required as well.
		...
		// These are the windows that by default are shown to all users.
		// However, to
		...
		return mContext
				.checkCallingOrSelfPermission(android.Manifest.permission.INTERNAL_SYSTEM_WINDOW) != PackageManager.PERMISSION_GRANTED;
	}
	...
	public void adjustConfigurationLw(Configuration config,
			int keyboardPresence, int navigationPresence) {
	...
	public int windowTypeToLayerLw(int type) {
		...
		case TYPE_WALLPAPER:
			// wallpaper is at the bottom, though the window manager may move
			// it.
			return 2;
		...
		case TYPE_PRIORITY_PHONE:
			// SIM errors and unlock. Not sure if this really should be in a
			// high layer.
			return 7;
		...
		case TYPE_INPUT_METHOD:
			// on-screen keyboards and other such input method user interfaces
			// go here.
			return 10;
		...
		case TYPE_KEYGUARD_SCRIM:
			// the safety window that shows behind keyguard while keyguard is
			// starting
			return 12;

		case TYPE_KEYGUARD:
			// the keyguard; nothing on top of these can take focus, since they
			// are
		...
		case TYPE_NAVIGATION_BAR_PANEL:
			// some panels (e.g. search) need to show on top of the navigation
			// bar
			return 21;
		...
		case TYPE_DRAG:
			// the drag layer: input for drag-and-drop is associated with this
			// window,
	...
	public int getNonDecorDisplayWidth(int fullWidth, int fullHeight,
			int rotation) {
		if (mHasNavigationBar) {
			// For a basic navigation bar, when we are in landscape mode we
			// place
	...
	public int getNonDecorDisplayHeight(int fullWidth, int fullHeight,
			int rotation) {
		if (mHasNavigationBar) {
	...
	public int getConfigDisplayHeight(int fullWidth, int fullHeight,
			int rotation) {
		// There is a separate status bar at the top of the display. We don't
		// count that as part
		// of the fixed decor, since it can hide; however, for purposes of
		// configurations,
		// we do want to exclude it since applications can't generally use that
		// part
		// of the screen.
	...
	@Override
	public boolean doesForceHide(WindowState win,
		...
		try {
			...
			if (DEBUG_STARTING_WINDOW)
				Slog.d(TAG, "addStartingWindow " + packageName
						...
						+ " theme=" + Integer.toHexString(theme));
			...
			if (ta.getBoolean(
					com.android.internal.R.styleable.Window_windowDisablePreview,
					false)
					|| ta.getBoolean(
			...
			if (DEBUG_STARTING_WINDOW)
				Slog.d(TAG, "Adding starting window for " + packageName + " / "
						+ appToken + ": " + (view.getParent() != null ? view : null));
	 ...
	 * @param attrs
	 *            Information about the window to be added
	 * 
	...
	public int prepareAddWindowLw(WindowState win,
		...
		case TYPE_STATUS_BAR:
			mContext.enforceCallingOrSelfPermission(
					android.Manifest.permission.STATUS_BAR_SERVICE,	"PhoneWindowManager");
			if (mStatusBar != null) {
	...
	public void removeWindowLw(WindowState win) {
		...
		// / M: [ALPS00799337] Dereference mTopFullscreenOpaqueWindowState when
		// remove it @{
	...
	public void selectRotationAnimationLw(int anim[]) {
		if (PRINT_ANIM)
			Slog.i(TAG,	"selectRotationAnimation mTopFullscreen="
							+ mTopFullscreenOpaqueWindowState
							+ " rotationAnimation="
							+ (mTopFullscreenOpaqueWindowState == null ? "0"
									: mTopFullscreenOpaqueWindowState
											.getAttrs().rotationAnimation));
	...
	public Animation createForceHideEnterAnimation(boolean onWallpaper) {
		return AnimationUtils
				.loadAnimation( mContext, onWallpaper ? com.android.internal.R.anim.lock_screen_wallpaper_behind_enter
	...
	public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event,
		...
		Log.d(TAG, 
				"interceptKeyTi keyCode=" + keyCode 
				+ " down=" + down
				+ " repeatCount=" + repeatCount 
				+ " keyguardOn=" + keyguardOn
				+ " mHomePressed=" + mHomePressed 
				+ " canceled=" + canceled
				+ " metaState:" + metaState);
...
//		gestureWakeupAction(keyCode, wakeIntent);
		...
		if (keyCode == KeyEvent.KEYCODE_HOME) {
			...
			if (win != null && win.getAttrs() != null) {
				...
				if ((flag & WindowManager.LayoutParams.FLAG_HOMEKEY_DISPATCHED) != 0) {
					// the window wants to handle the home key, so dispatch it
					// to it.
					return 0;
				}
			}
			...
			if (!down) {
				...
				if (mDoubleTapOnHomeBehavior != DOUBLE_TAP_HOME_NOTHING) {
					mHandler.removeCallbacks(mHomeDoubleTapTimeoutRunnable); // just
																				// in
																				// case
			...
			if (down) {
				...
				int auto = Settings.System.getIntForUser(mContext.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS_MODE,
				...
				int step = (max - min + BRIGHTNESS_STEPS - 1)
						/ BRIGHTNESS_STEPS * direction;
		...
		if (FeatureOption.MTK_SMARTBOOK_SUPPORT && mSmartBookPlugIn) {
			if (down && repeatCount == 0 && keyCode == KeyEvent.KEYCODE_WIFI_TOGGLE) {
				...
				if (!mKeyguardDelegate.isShowingAndNotHidden()) {
			...
			} else if (down	&& repeatCount == 0 && (keyCode == KeyEvent.KEYCODE_SHIFT_LEFT && (metaState & KeyEvent.META_CTRL_ON) != 0)) {
				mWindowManagerFuncs.switchInputMethod(true);
				return -1;
			} else if (down	&& repeatCount == 0 && (keyCode == KeyEvent.KEYCODE_SHIFT_RIGHT && (metaState & KeyEvent.META_CTRL_ON) != 0)) {
		...
		if (mSearchKeyShortcutPending) {
			...
			if (kcm.isPrintingKey(keyCode)) {
				...
				if (down && repeatCount == 0 && !keyguardOn) {
					Intent shortcutIntent = mShortcutManager.getIntent(kcm,
							keyCode, metaState);
					if (shortcutIntent != null) {
						shortcutIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
						try {
							mContext.startActivityAsUser(shortcutIntent,
									UserHandle.CURRENT);
		...
		if (down && repeatCount == 0 && !keyguardOn	&& (metaState & KeyEvent.META_META_ON) != 0) {
			final KeyCharacterMap kcm = event.getKeyCharacterMap();
			if (kcm.isPrintingKey(keyCode)) {
				Intent shortcutIntent = mShortcutManager
						.getIntent(
								kcm,
								keyCode,
								metaState & ~(KeyEvent.META_META_ON | KeyEvent.META_META_LEFT_ON | KeyEvent.META_META_RIGHT_ON));
				if (shortcutIntent != null) {
					shortcutIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					try {
						mContext.startActivityAsUser(shortcutIntent,
		...
		if (down && repeatCount == 0 && keyCode == KeyEvent.KEYCODE_TAB) {
			if (mRecentAppsDialogHeldModifiers == 0 && !keyguardOn) {
				...
				if (KeyEvent.metaStateHasModifiers(shiftlessModifiers, KeyEvent.META_ALT_ON)
						|| KeyEvent.metaStateHasModifiers(shiftlessModifiers, KeyEvent.META_META_ON)) {
			...
			showOrHideRecentAppsDialog(
				keyguardOn ? 
				RECENT_APPS_BEHAVIOR_DISMISS : RECENT_APPS_BEHAVIOR_DISMISS_AND_SWITCH);
		}
	...
	public KeyEvent dispatchUnhandledKey(WindowState win, KeyEvent event,
		...
		if (DEBUG_INPUT) {
			Slog.d(TAG,	"Unhandled key: win=" + win + ", action=" + event.getAction() + ", flags=" + event.getFlags()
	...
	private boolean interceptFallback(WindowState win, KeyEvent fallbackEvent,
			int policyFlags) {
	...
	private void launchAssistAction() {
		sendCloseSystemWindows(SYSTEM_DIALOG_REASON_ASSIST);
		Intent intent = ((SearchManager) mContext.getSystemService(Context.SEARCH_SERVICE)).getAssistIntent(
				mContext, true, UserHandle.USER_CURRENT);
		if (intent != null) {
	...
	private void toggleRecentApps() {
		mPreloadedRecentApps = false; // preloading no longer needs to be
										// canceled
	...
	final class HideNavInputEventReceiver extends InputEventReceiver {
		public HideNavInputEventReceiver(InputChannel inputChannel,
				Looper looper) {
			...
			try {
				if (event instanceof MotionEvent && (event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
					...
					if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
						...
						synchronized (mWindowManagerFuncs
								.getWindowManagerLock()) {
							// Any user activity always causes us to show the
							// navigation controls, if they had been hidden.
							// We also clear the low profile and only content
							// flags so that tapping on the screen will
							// atomically
							...
							if (mForceClearedSystemUiFlags != newVal) {
								...
								changed = true;
								mHandler.postDelayed(mClearHideNavigationFlag,
										1000);
							}
						}
						if (changed) {
	...
	public void getContentInsetHintLw(WindowManager.LayoutParams attrs,	Rect contentInset) {
		...
		if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR)) == (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR)) {
			...
			if (canHideNavigationBar() && (systemUiVisibility & View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION) != 0) {
				...
				availBottom = mUnrestrictedScreenTop
			...
			if ((systemUiVisibility & View.SYSTEM_UI_FLAG_LAYOUT_STABLE) != 0) {
				if ((fl & FLAG_FULLSCREEN) != 0) {
					...
					contentInset.set(mStableLeft, mStableTop, availRight
							- mStableRight, availBottom - mStableBottom);
				}
			} else if ((fl & FLAG_FULLSCREEN) != 0
					|| (fl & FLAG_LAYOUT_IN_OVERSCAN) != 0) {
				contentInset.setEmpty();
			} else if ((systemUiVisibility & (View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN)) == 0) {
				contentInset.set(mCurLeft, mCurTop, availRight - mCurRight,
						availBottom - mCurBottom);
			} else {
				contentInset.set(mCurLeft, mCurTop, availRight - mCurRight,
						availBottom - mCurBottom);
			}
	...
	public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth,
		...
		if (isDefaultDisplay) {
			// For purposes of putting out fake window up to steal focus, we
			// will
			...
			// decided that it can't be hidden (because of the screen aspect
			// ratio),
			...
			if (mNavigationBar != null) {
				...
				if (mNavigationBarOnBottom) {
					// It's a system nav bar or a portrait screen; nav bar goes
					// on bottom.
					...
					if (navVisible
						...
						// If the opaque nav bar is currently requested to be
						// visible,
			...
			if (DEBUG_LAYOUT)
				Slog.i(TAG, 
						String.format("mDock rect: (%d,%d - %d,%d)",
			...
			if (!mLayoutDisableStatusBar) {
				...
				// For layout, the status bar is always at the top with our
				// fixed height.
				...
				if (mStatusBar.isVisibleLw() && !statusBarTransient) {
					...
					if (DEBUG_LAYOUT)
						Slog.v(TAG, "Status bar: " + String.format(
												"dock=[%d,%d][%d,%d] content=[%d,%d][%d,%d] cur=[%d,%d][%d,%d]",
												mDockLeft, mDockTop,
												mDockRight, mDockBottom,
												mContentLeft, mContentTop,
												mContentRight, mContentBottom,
												mCurLeft, mCurTop, mCurRight,
												mCurBottom));
				}
				if (mStatusBar.isVisibleLw() && !mStatusBar.isAnimatingLw()
					...
					// If the opaque status bar is currently requested to be
					// visible,
					// and not in the process of animating on or off, then
					// we can tell the app that it is covered by it.
			...
			// / M: [ALPS00411109] The statu bar disappear and a blank area
			// displayed. @{
	...
	void setAttachedWindowFrames(WindowState win, int fl, int adjust,
			...
			Rect of, Rect cf, Rect vf) {
		if (win.getSurfaceLayer() > mDockLayer
				&& attached.getSurfaceLayer() < mDockLayer) {
	...
	public void layoutWindowLw(WindowState win,
		...
		if (!isDefaultDisplay) {
			if (attached != null) {
				...
				setAttachedWindowFrames(win, fl, adjust, attached, true, pf,
						df, of, cf, vf);
			} else {
			...
			final boolean topAtRest = win == mTopFullscreenOpaqueWindowState
					&& !win.isAnimatingLw();
			if (isAppWindow && !inheritTranslucentDecor && !topAtRest) {
				if ((sysUiFl & View.SYSTEM_UI_FLAG_FULLSCREEN) == 0
					...
					// / M: [ALPS01237089]Fix sogou fullscreen activity layout
					// issue
					dcf.top = mDockTop;
				}
			...
			if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR)) == (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR)) {
				if (DEBUG_LAYOUT)
					Log.v(TAG, "layoutWindowLw(" + attrs.getTitle()	+ "): IN_SCREEN, INSET_DECOR, sim = #"
									+ Integer.toHexString(adjust));
				...
				if (attached != null) {
					// If this window is attached to another, our display
					// frame is the same as the one we are attached to.
					setAttachedWindowFrames(win, fl, adjust, attached, true,
							pf, df, of, cf, vf);
				} else {
					if (attrs.type == TYPE_STATUS_BAR_PANEL	|| attrs.type == TYPE_STATUS_BAR_SUB_PANEL) {
						...
						if (DEBUG_LAYOUT)
							Slog.v(TAG,	String.format("Laying out status bar window: (%d,%d - %d,%d)",
											pf.left, pf.top, pf.right,
											pf.bottom));
					} else if ((fl & FLAG_LAYOUT_IN_OVERSCAN) != 0
							&& attrs.type >= WindowManager.LayoutParams.FIRST_APPLICATION_WINDOW
							&& attrs.type <= WindowManager.LayoutParams.LAST_SUB_WINDOW) {
						// Asking to layout into the overscan region, so give it
						// that pure
						// unrestricted area.
						...
						pf.bottom = df.bottom = of.bottom = mOverscanScreenTop + mOverscanScreenHeight;
							...
							&& (attrs.type == WindowManager.LayoutParams.TYPE_KEYGUARD || (attrs.type >= WindowManager.LayoutParams.FIRST_APPLICATION_WINDOW && attrs.type <= WindowManager.LayoutParams.LAST_SUB_WINDOW))) {
						// Asking for layout as if the nav bar is hidden, lets
						// the
						// application extend into the unrestricted overscan
						// screen area. We
						// only do this for application windows to ensure no
						// window that
						...
						// We need to tell the app about where the frame inside
						// the overscan
						// is, so it can inset its content by that amount -- it
						// didn't ask
					...
					if ((fl & FLAG_FULLSCREEN) == 0) {
						...
						// / M: [ALPS00569504] [ALPS00411109] [ALPS00435427] The
						// statu bar disappear and a blank area displayed. @{
						if (mfindingTopFullscreenOpaque
								&& !mLayoutDisableStatusBar) {
						...
						// Full screen windows are always given a layout that is
						// as if the
						// status bar and other transient decors are gone. This
						// is to avoid
						// bad states when moving from a window that is not
						// hding the
					...
					|| (sysUiFl & (View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION)) != 0) {
				if (DEBUG_LAYOUT)
				...
				if (attrs.type == TYPE_STATUS_BAR_PANEL || attrs.type == TYPE_STATUS_BAR_SUB_PANEL) {
					...
					pf.right = df.right = of.right = cf.right = hasNavBar ? mRestrictedScreenLeft + mRestrictedScreenWidth
							: mUnrestrictedScreenLeft + mUnrestrictedScreenWidth;
					pf.bottom = df.bottom = of.bottom = cf.bottom = hasNavBar ? mRestrictedScreenTop + mRestrictedScreenHeight
							: mUnrestrictedScreenTop + mUnrestrictedScreenHeight;
					if (DEBUG_LAYOUT)
					...
					pf.bottom = df.bottom = of.bottom = cf.bottom = mOverscanScreenTop + mOverscanScreenHeight;
						...
						|| attrs.type == TYPE_TOP_MOST || (attrs.type >= WindowManager.LayoutParams.FIRST_APPLICATION_WINDOW && attrs.type <= WindowManager.LayoutParams.LAST_SUB_WINDOW))) {
					// Asking for layout as if the nav bar is hidden, lets the
					// application extend into the unrestricted screen area. We
					// only do this for application windows (or toasts) to
					// ensure no window that
					// can be above the nav bar can do this.
					// XXX This assumes that an app asking for this will also
					// ask for layout in only content. We can't currently figure
					// out
					// what the screen would be if only laying out to hide the
					// nav bar.
					pf.left = df.left = of.left = cf.left = mUnrestrictedScreenLeft;
					pf.top = df.top = of.top = cf.top = mUnrestrictedScreenTop;
					pf.right = df.right = of.right = cf.right = mUnrestrictedScreenLeft + mUnrestrictedScreenWidth;
				...
				// / M: [ALPS00569504] [ALPS00435427] The statu bar disappear
				// and a blank area displayed.
				// / [ALPS00449990] The picture shakes when tap the screen in
				// single view @{
				boolean nonFullScreenApplyStable = (sysUiFl & View.SYSTEM_UI_FLAG_LAYOUT_STABLE) != 0 && (fl & FLAG_FULLSCREEN) == 0;
				if (mfindingTopFullscreenOpaque && !nonFullScreenApplyStable && !mLayoutDisableStatusBar) {
				...
				if (attrs.type == TYPE_STATUS_BAR_PANEL) {
					...
					pf.right = df.right = of.right = cf.right = mRestrictedScreenLeft + mRestrictedScreenWidth;
					pf.bottom = df.bottom = of.bottom = cf.bottom = mRestrictedScreenTop + mRestrictedScreenHeight;
				} else if (attrs.type == TYPE_TOAST
						|| attrs.type == TYPE_SYSTEM_ALERT) {
		...
		// TYPE_SYSTEM_ERROR is above the NavigationBar so it can't be allowed
		// to extend over it.
		...
		if (DEBUG_LAYOUT)
			Slog.v(TAG,	"Compute frame " + attrs.getTitle() + ": sim=#"
							+ Integer.toHexString(sim) + " attach=" + attached
							+ " type=" + attrs.type
							+ String.format(" flags=0x%08x", fl) + " pf="
							+ pf.toShortString() + " df=" + df.toShortString()
							+ " of=" + of.toShortString() + " cf="
							+ cf.toShortString() + " vf=" + vf.toShortString()
		...
		// / M: [ALPS01250663][ALPS00805067]Let IME involve visibleFrame
		// computation earlier
		...
		if (mfindingTopFullscreenOpaque && win.isVisibleLw()) {
			if (attrs.type >= FIRST_APPLICATION_WINDOW
					&& attrs.type <= LAST_APPLICATION_WINDOW && attrs.x == 0
					&& attrs.y == 0
	...
	private void offsetInputMethodWindowLw(WindowState win) {
		...
		if (DEBUG_LAYOUT)
			Slog.v(TAG, "Input method: mDockBottom=" + mDockBottom
	...
	public void beginPostLayoutPolicyLw(int displayWidth, int displayHeight) {
		...
		// / M: [ALPS00445635] The idle screen will flash a short time when tap
		// Emergency call @{
	...
	public void applyPostLayoutPolicyLw(WindowState win,
		...
		if (mTopFullscreenOpaqueWindowState == null
				&& (win.getAttrs().privateFlags & WindowManager.LayoutParams.PRIVATE_FLAG_FORCE_SHOW_NAV_BAR) != 0
				|| (win.isVisibleLw() 
		...
		if (mTopFullscreenOpaqueWindowState == null	&& win.isVisibleOrBehindKeyguardLw() && !win.isGoneForLayoutLw()) {
			...
			if (attrs.type == TYPE_DREAM) {
				// If the lockscreen was showing when the dream started then
				// wait
			...
			if (applyWindow && attrs.x == 0 && attrs.y == 0
					&& attrs.width == WindowManager.LayoutParams.MATCH_PARENT
					&& attrs.height == WindowManager.LayoutParams.MATCH_PARENT) {
				...
				if ((attrs.flags & FLAG_SHOW_WHEN_LOCKED) != 0) {
					...
					// / M: [ALPS00445635] The idle screen will flash a short
					// time when tap Emergency call @{
					if (DEBUG_LAYOUT)
						Log.v(TAG, "Setting mHideLockScreen to false by win " + win);
				...
				if ((attrs.flags & FLAG_DISMISS_KEYGUARD) != 0
					...
					// / M: [ALPS00484759] Handle switch Keyguard secure mode
					// when dismissing @{
					final boolean isSecure = mKeyguardDelegate != null ? mKeyguardDelegate
							.isSecure() : false;
					mDismissKeyguard = mWinDismissingKeyguard == win
							&& mSecureDismissingKeyguard == isSecure ? DISMISS_KEYGUARD_CONTINUE
	...
	public int finishPostLayoutPolicyLw() {
		...
		if (!mLayoutDisableStatusBar) {
			if (DEBUG_LAYOUT)
				Slog.i(TAG, "force=" + mForceStatusBar 
			...
			if (mForceStatusBar || mForceStatusBarFromKeyguard) {
				...
				// Maintain fullscreen layout until incoming animation is
				// complete.
				topIsFullscreen = mTopIsFullscreen && mStatusBar.isAnimatingLw();
				// Transient status bar on the lockscreen is not allowed
				if (mForceStatusBarFromKeyguard && mStatusBarController.isTransientShowing()) {
					mStatusBarController.updateVisibilityLw(
							false /* transientAllowed */, mLastSystemUiFlags,
							mLastSystemUiFlags);
				}
				...
				if (localLOGV) {
					...
					Slog.d(TAG,
							"attr: " + mTopFullscreenOpaqueWindowState.getAttrs() 
		...
		if (mKeyguard != null) {
			...
			if (mDismissKeyguard != DISMISS_KEYGUARD_NONE && !mKeyguardDelegate.isSecure()) {
				...
				// This is the case of keyguard isSecure() and not
				// mHideLockScreen.
				if (mDismissKeyguard == DISMISS_KEYGUARD_START) {
					// Only launch the next keyguard unlock window once per
					// window.
				...
				// / M: [ALPS00484759] Handle switch Keyguard secure mode when
				// dismissing @{
				mSecureDismissingKeyguard = false;
				// / @}
				// / M: [ALPS00082277] In order to enhanced the showing
				// performance
	...
	public boolean allowAppAnimationsLw() {
		if (mKeyguard != null && mKeyguard.isVisibleLw()
				&& !mKeyguard.isAnimatingLw() || mShowingDream) {
			// If keyguard or dreams is currently visible, no reason to animate
			// behind it.
			return false;
		}
		return true;
	...
	public int focusChangedLw(WindowState lastFocus, WindowState newFocus) {
		...
		// / M: [ALPS00399816] Receive one call during changed Input Method,can
		// not display keyboard after end call @{
	...
	/**
	 * @return Whether music is being played right now "locally" (e.g. on the
	 *         device's speakers or wired headphones) or "remotely" (e.g. on a
	 *         device using the Cast protocol and controlled by this device, or
	 *         through remote submix).
	 */
	...
	void handleVolumeKey(int stream, int keycode) {
		...
		try {
			...
			if (stream == AudioSystem.STREAM_MUSIC) {
				audioService.adjustLocalOrRemoteStreamVolume(
								stream,
								keycode == KeyEvent.KEYCODE_VOLUME_UP ? AudioManager.ADJUST_RAISE
										: AudioManager.ADJUST_LOWER, mContext
										.getOpPackageName());
			} else {
	...
	private Runnable mKeyRemappingVolumeDownLongPress_Test = new Runnable() {
		public void run() {
			...
			try {
				...
				Log.d(TAG, "**** Exception: Thread Sleeping ");
			}
	...
	public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags,
		...
		final boolean keyguardActive = (mKeyguardDelegate == null ? false
				: (isScreenOn ? mKeyguardDelegate.isShowingAndNotHidden()
		...
		Log.d(TAG, "interceptKeyTq keycode=" + keyCode + " screenIsOn="
				+ isScreenOn + " keyguardActive=" + keyguardActive
		...
		if (FeatureOption.MTK_HW_KEY_REMAPPING) {
			...
			if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
				...
				if (action == KeyEvent.ACTION_DOWN) {
					mHandler.postDelayed(mKeyRemappingVolumeUpLongPress,
							ViewConfiguration.getGlobalActionKeyTimeout());
				...
				if (action == KeyEvent.ACTION_DOWN) {
					mHandler.postDelayed(mKeyRemappingVolumeDownLongPress,
							ViewConfiguration.getGlobalActionKeyTimeout());
				} else if (action == KeyEvent.ACTION_UP) {
					mHandler.removeCallbacks(mKeyRemappingVolumeDownLongPress);
		...
		case KeyEvent.KEYCODE_VOLUME_MUTE: {
			...
			if (down) {
				...
				if (telephonyService != null) {
					try {
						if (telephonyService.isRinging()) {
							// If an incoming call is ringing, either VOLUME key
							// means
							// "silence ringer". We handle these keys here,
							// rather than
							// in the InCallScreen, to make sure we'll respond
							// to them
							// even if the InCallScreen hasn't come to the
							// foreground yet.
							// Look for the DOWN event here, to agree with the
							// "fallback"
						...
						if (telephonyService.isOffhook()
							...
							// / M: [ALPS00462787] HFP,HFP Volume cannot change
				...
				if (isMusicActive() && (result & ACTION_PASS_TO_USER) == 0) {
					// If music is playing but we decided not to pass the key to
					// the
				...
				// / M: [ALPS00270178] Adjust Volume key for FM at
				// PhoneWindowManager.java
				if (isFmActive() && (result & ACTION_PASS_TO_USER) == 0) {
					// If FM is playing but we decided not to pass the key to
					// the
					// application, handle the volume change here.
				...
				if (telephonyService != null) {
					try {
						if (!telephonyService.isIdle()) {
							// Suppress PLAY/PAUSE toggle when phone is ringing
							// or in-call
		...
		case KeyEvent.KEYCODE_MEDIA_AUDIO_TRACK: {
			if ((result & ACTION_PASS_TO_USER) == 0) {
				...
				// In that
				// case, the PhoneWindow class will do the same thing, except it
				// will
				// only do it if the showing app doesn't process the key on its
				// own.
				// Note that we need to make a copy of the key event here
				// because the
				// original key event will be recycled when we return.

				mBroadcastWakeLock.acquire();
				Message msg = mHandler.obtainMessage(
						MSG_DISPATCH_MEDIA_KEY_WITH_WAKE_LOCK, new KeyEvent(
								event));
	 ...
	 * it is always considered a wake key.
	 */
	...
	public int interceptMotionBeforeQueueingWhenScreenOff(int policyFlags) {
		...
		final boolean isWakeMotion = (policyFlags & (WindowManagerPolicy.FLAG_WAKE | WindowManagerPolicy.FLAG_WAKE_DROPPED)) != 0;
		if (isWakeMotion) {
			result |= ACTION_WAKE_UP;
		}
		return result;
	...
	void dispatchMediaKeyWithWakeLock(KeyEvent event) {
		...
		if (mHavePendingMediaKeyRepeatWithWakeLock) {
			...
			mBroadcastWakeLock.release(); // pending repeat was holding onto the
											// wake lock
		}
	...
	BroadcastReceiver mStkUserActivityEnReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			Log.v(TAG, 
					"mStkUserActivityEnReceiver -- onReceive -- entry");
			...
			if (DEBUG_INPUT) {
				Log.v(TAG, "mStkUserActivityEnReceiver -- onReceive -- exist " + mIsStkUserActivityEnabled);
			}
		}
	};
	...
	BroadcastReceiver mDreamReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			} else if (Intent.ACTION_DREAMING_STOPPED
	...
	private void waitForKeyguard(final ScreenOnListener screenOnListener) {
		if (mKeyguardDelegate != null) {
			if (screenOnListener != null) {
				mKeyguardDelegate
						.onScreenTurnedOn(new KeyguardServiceDelegate.ShowListener() {
							@Override
							public void onShown(IBinder windowToken) {
								// / M: [ALPS00428805] Avoid to output a temp
								// screen if there is no keyguard when screen
								// on. @{
								if (windowToken == null) {
									waitForTopFullscreenWindowDrawn(screenOnListener);
								} else {
									waitForKeyguardWindowDrawn(windowToken,
											screenOnListener);
								}
								// / @}
							}
		...
		// / M: [ALPS00428805] Avoid to output a temp screen if there is no
		// keyguard when screen on. @{
		// finishScreenTurningOn(screenOnListener);
		waitForTopFullscreenWindowDrawn(screenOnListener);
		// / @}
	}

	// / M: [ALPS00428805] Avoid to output a temp screen if there is no keyguard
	// when screen on. @{
	private void waitForTopFullscreenWindowDrawn(
	...
	private void waitForKeyguardWindowDrawn(IBinder windowToken,
		...
		if (windowToken != null && !mHideLockScreen) {
			try {
				if (mWindowManager.waitForWindowDrawn(windowToken,
				...
				Slog.i(TAG,
						 "No lock screen! waitForWindowDrawn false");

			} catch (RemoteException ex) {
				// Can't happen in system process.
				// / M:ALPS01290677: Avoid JE if user immediately finish
				// activity after screen on
		...
		Slog.i(TAG,
				 "No lock screen! windowToken=" + windowToken);
		finishScreenTurningOn(screenOnListener);
	}
			...
			mHandler.post(new Runnable() {
				public void run() {
					if (mKeyguardDelegate.isDismissable()) {
						...
						// ask the keyguard to prompt the user to authenticate
						// if necessary
						mKeyguardDelegate.dismiss();
					}
				}
		...
		synchronized (mLock) {
			...
			if (mSmartBookPlugIn) {
				...
				preferredRotation = mCarDockEnablesAccelerometer ? sensorRotation : mCarDockRotation;
					...
					|| mDockMode == Intent.EXTRA_DOCK_STATE_LE_DESK || mDockMode == Intent.EXTRA_DOCK_STATE_HE_DESK)
				...
				// Ignore sensor when plugged into HDMI when demo HDMI rotation
				// lock enabled.
				...
				// Ignore sensor when plugged into HDMI and an undocked
				// orientation has
				// been specified in the configuration (only for legacy devices
				// without
				...
				if (mAllowAllRotations < 0) {
					...
					mAllowAllRotations = mContext
							.getResources()
							.getBoolean(
									com.android.internal.R.bool.config_allowAllRotations) ? 1
							: 0;
				}
				...
				// The idea is that the user rotation expresses a weak
				// preference for the direction
				// of gravity and as NOSENSOR is never affected by gravity, then
				// neither should
				// NOSENSOR be affected by rotation lock (although it will be
				// affected by docks).
			...
			if (DEBUG_ORIENTATION) {
				Slog.v(TAG, "rotationForOrientationLw(appReqQrientation = "
						...
						+ ", UserRotation = " + mUserRotation + ", LidState = "
						+ mLidState + ", DockMode = " + mDockMode
	...
	public void setSafeMode(boolean safeMode) {
		mSafeMode = safeMode;
		performHapticFeedbackLw(null,
				safeMode ? HapticFeedbackConstants.SAFE_MODE_ENABLED
		...
		mHandler.post(new Runnable() {
			...
			public void run() {
				if (mBootMsgDialog == null) {
					...
					WindowManager.LayoutParams lp = mBootMsgDialog.getWindow()
							.getAttributes();
	...
	public void userActivity() {
		...
		// / M:[ALPS00062902]When the user activiy flag is enabled, it notifies
		// the intent "STK_USERACTIVITY"
		// / @{
		synchronized (mStkLock) {
	...
	public void lockNow(Bundle options) {
		...
		mHandler.removeCallbacks(mScreenLockTimeout);
		if (options != null) {
			// In case multiple calls are made to lockNow, we don't wipe out the
			// options
	...
	private void updateLockScreenTimeout() {
		synchronized (mScreenLockTimeout) {
			boolean enable = (mAllowLockscreenWhenOn && mScreenOnEarly
					&& mKeyguardDelegate != null && mKeyguardDelegate
					.isSecure());
	...
	// add-s by minesea 2014/8/6 9:39:27 for add lock wake
	public void sendUnLockScreen() {
		/*
		 * mKeyguardDelegate.verifyUnlock(new OnKeyguardExitResult() { public
		 * void onKeyguardExitResult(boolean success) { if (success) { try {
		 * ActivityManagerNative.getDefault().stopAppSwitches(); } catch
		 * (RemoteException e) { Log.e("leihai", "sendUnLockScreen  -- " + e); }
		 * } } });
		 */
		/*
...
//		});
		...
		if (false) {
			...
			try {
				if (SystemProperties.getInt("persist.sys.uts-test-mode", 0) == 1) {
					...
					Intent dock = createHomeDockIntent();
					if (dock != null) {
						int result = ActivityManagerNative
								.getDefault()
	...
	private boolean isGlobalAccessibilityGestureEnabled() {
		return Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.ENABLE_ACCESSIBILITY_GLOBAL_GESTURE_ENABLED, 0) == 1;
	}
	...
	public boolean performHapticFeedbackLw(WindowState win, int effectId,
		...
		final boolean hapticsDisabled = Settings.System.getIntForUser(
				mContext.getContentResolver(), Settings.System.HAPTIC_FEEDBACK_ENABLED, 0, UserHandle.USER_CURRENT) == 0;
		if (!always
				&& (hapticsDisabled || mKeyguardDelegate.isShowingAndNotHidden())) {
			return false;
		}
		long[] pattern = null;
		switch (effectId) {
	...
	private int updateSystemUiVisibilityLw() {
		...
		int tmpVisibility = win.getSystemUiVisibility()	& ~mResettingSystemUiFlags & ~mForceClearedSystemUiFlags;
	...
	private int updateSystemBarsLw(WindowState win, int oldVis, int vis) {
		...
		// WindowState transWin = mKeyguard != null && mKeyguard.isVisibleLw()
		// && !mHideLockScreen
		// ? mKeyguard
		// : mTopFullscreenOpaqueWindowState;
		// M: [ALPS01259584] Update status bar by top window when Keyguard
		// animating.
		WindowState transWin = mKeyguard != null
				&& (mKeyguard.isVisibleLw() && !mKeyguard.isAnimatingLw())
				&& !mHideLockScreen ? mKeyguard	: mTopFullscreenOpaqueWindowState;
		...
		boolean hideStatusBarWM = mTopFullscreenOpaqueWindowState != null
				&& (mTopFullscreenOpaqueWindowState.getAttrs().flags & WindowManager.LayoutParams.FLAG_FULLSCREEN) != 0;
		...
		vis = mStatusBarController.updateVisibilityLw(
				transientStatusBarAllowed, oldVis, vis);
	...
	private boolean isImmersiveMode(int vis) {
		...
		return mNavigationBar != null
				...
				&& (vis & flags) != 0 && canHideNavigationBar();
	}

	/**
	 * @return whether the navigation or status bar can be made translucent
	 * 
	 *         This should return true unless touch exploration is not enabled
	 *         or R.boolean.config_enableTranslucentDecor is false.
	 */
	...
	private void killRunningProcessForSMB() {
		...
		try {
			final IActivityManager am = ActivityManagerNative
					.asInterface(ServiceManager.checkService("activity"));
			...
			for (RunningAppProcessInfo ri : runningApps) {
				...
				for (ApplicationInfo ai : installedApps) {
					...
					if (ri.processName.equals(ai.processName)) {// kill
																// 3rd-party
																// process
				...
				for (int i = 0; i < sBlackList.size(); ++i) {
					if (ri.processName.contains(sBlackList.get(i))) {// kill
																		// black
																		// list
																		// process
						Process.killProcess(ri.pid);
	...
	public void dump(String prefix, PrintWriter pw, String[] args) {
		...
		if (args.length > 0 && args[0].contains("-d")) {
			...
			// / @}
		} else if (args.length > 0 && args[0].contains("-sb")) {
			if (mSmartBookPlugIn) {
				Slog.d(TAG, "setHdmiPlugged: false");
				setHdmiPlugged(false);
			} else {
				Slog.d(TAG, "setHdmiPlugged: true");
				setHdmiPlugged(true);
			}
		} else {
			pw.print(prefix);
...
//ccx add
	private void gestureWakeupAction(int keyCode, Intent wakeIntent){
		int isGestureWakeup = Settings.System.getInt(mContext.getContentResolver(),
				KEY_GESTURE_WAKEUP_VALUE, 1);
        int isGestureSmsWakeup = Settings.System.getInt(mContext.getContentResolver(),
				KEY_GESTURE_SMS_VALUE, 1);
		int isGestureMusicWakeup = Settings.System.getInt(mContext.getContentResolver(),
				KEY_GESTURE_MUSIC_VALUE, 1);
		int isGestureCameraWakeup = Settings.System.getInt(mContext.getContentResolver(),
				KEY_GESTURE_CAMERA_VALUE, 1);
		int isGestureBrowserWakeup = Settings.System.getInt(mContext.getContentResolver(),
				KEY_GESTURE_BROWSER_VALUE, 1);
		int isGestureCallWakeup = Settings.System.getInt(mContext.getContentResolver(),
				KEY_GESTURE_CALL_VALUE, 1);

		Log.i("cui_Log", "gestureWakeupAction() isGestureWakeup="+isGestureWakeup
											+"; isGestureSmsWakeup="+isGestureSmsWakeup
											+"; isGestureMusicWakeup="+isGestureMusicWakeup
											+"; isGestureCameraWakeup="+isGestureCameraWakeup
											+"; isGestureBrowserWakeup="+isGestureBrowserWakeup
											+"; isGestureCallWakeup="+isGestureCallWakeup
											);
		if (0 == isGestureWakeup){
			return;
		}
		
		// wwj add
		// 'w' message
		if (keyCode == KeyEvent.KEYCODE_S && 1 == isGestureSmsWakeup) {
			ComponentName comp = new ComponentName("com.android.mms",
					"com.android.mms.ui.BootActivity");
			wakeIntent.setComponent(comp);
			mContext.startActivity(wakeIntent);
			sendUnLockScreen();
		}
		// 'c' photograph
		else if (keyCode == KeyEvent.KEYCODE_C && 1 == isGestureCameraWakeup) {
		...
		// 'e' explorer
		else if (keyCode == KeyEvent.KEYCODE_E && 1 == isGestureBrowserWakeup) {
		...
		// 'o' dialing
		else if (keyCode == KeyEvent.KEYCODE_O && 1 == isGestureCallWakeup) {
		...
		// 'm' contacts
		else if (keyCode == KeyEvent.KEYCODE_M && 1 == isGestureMusicWakeup) {
