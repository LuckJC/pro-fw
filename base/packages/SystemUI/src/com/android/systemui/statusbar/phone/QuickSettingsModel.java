...
import com.android.systemui.statusbar.policy.BatteryController.LaptopBatteryStateChangeCallback;
...
class QuickSettingsModel implements BluetoothStateChangeCallback,
        ...
        LaptopBatteryStateChangeCallback,
    ...
    private BroadcastReceiver mAlarmIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_ALARM_CHANGED)) {
                ...
                onNextAlarmChanged();
            }/*else if(action.equals(COM_MINESEA_SENSOR_UPDATE_QUICK)){
            	boolean isSensorOpen = intent.getExtras().getBoolean("isOpen"); 
            	onSensorModeChanged(isSensorOpen);
            	}*/
        }
    };
    ...
    //add    
    private QuickSettingsTileView mFloatTile;
    private RefreshCallback mFloatCallback;
    private State mFloatState = new State();
    
    private QuickSettingsTileView mSensorTile;
    private RefreshCallback mSensorCallback;
    private State mSensorState = new State();
    ...
    void setUserTileInfo(String name, Drawable avatar) {
        /// M:
        Xlog.d("QuickSettingsModel","setUserTileInfo name is " + name);
    ...
    public void onLocationSettingsChanged(boolean locationEnabled) {
        ...
        Xlog.d("QuickSettingsModel","onLocationSettingsChanged enabled = " + locationEnabled);
        mLocationTile.setEnabled(true);
        mLocationCallback.refreshView(mLocationTile, mLocationState);
    }
    ...
    static class LaptopBatteryState extends State {
        int batteryLevel;
        boolean pluggedIn;
        boolean isPresent;
    }

    void addLaptopBatteryTile(QuickSettingsTileView view, RefreshCallback cb) {
        mLaptopBatteryTile = view;
        mLaptopBatteryCallback = cb;
        mLaptopBatteryCallback.refreshView(mLaptopBatteryTile, mLaptopBatteryState);
    }

    @Override
    public void onLaptopBatteryLevelChanged(int level, boolean pluggedIn, boolean isPresent) {
        mLaptopBatteryState.batteryLevel = level;
        mLaptopBatteryState.pluggedIn = pluggedIn;
        mLaptopBatteryState.isPresent = isPresent;
        mLaptopBatteryCallback.refreshView(mLaptopBatteryTile, mLaptopBatteryState);
    }
