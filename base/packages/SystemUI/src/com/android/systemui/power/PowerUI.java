...
public class PowerUI extends SystemUI {
    ...
    private Handler ledHandler = new LedHandler();
   	private final static int SHOWLED = 88;
    private boolean isShow = false;
    private static final int LEDMESSAGE = 66;
    //end add
    public void start() {
        ...
        mScreenOffTime = pm.isScreenOn() ? -1 : SystemClock.elapsedRealtime();
				nm = (NotificationManager) mContext.getSystemService(Context.NOTIFICATION_SERVICE);//add by gongjh
    ...
    //start add by gongjh 
		private void showLED() {
				...
				nm.notify(SHOWLED, notification);
			//
	...
	class LedHandler extends Handler{
		...
		public void handleMessage(Message msg) {
			...
			case LEDMESSAGE:
				if(isShow){
					System.out.println("dsds-->aa");
					isShow = false;
					nm.cancelAll();
				}else{
					System.out.println("dsds-->bb");
					showLED();
					isShow = true;
				}
				ledHandler.sendEmptyMessageDelayed(LEDMESSAGE, 1000L);
				break;
	...
	private void shutDownLed(){
		if(ledHandler != null){
			ledHandler.removeMessages(LEDMESSAGE);
		}
			nm.cancelAll();
	}
	//end add
    ...
    void showLowBatteryWarning() {
        ...
        if (mBatteryLevelTextView != null) {
            ...
            d.show();
           ledHandler.sendEmptyMessageDelayed(LEDMESSAGE, 1000L);//add by gongjh
            mLowBatteryDialog = d;
        }
    }
