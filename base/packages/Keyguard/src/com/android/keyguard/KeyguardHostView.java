...
public class KeyguardHostView extends KeyguardViewBase {
	...
	private boolean mEnableFallback; // TODO: This should get the value from
										// KeyguardPatternView
	...
	private static boolean IS_Tablet = ("tablet".equals(SystemProperties
	...
	// User for whom this host view was created. Final because we should never
	// change the
	// id without reconstructing an instance of KeyguardHostView. See note
	// below...
	...
	public KeyguardHostView(Context context, AttributeSet attrs) {
		...
		// Note: This depends on KeyguardHostView getting reconstructed every
		// time the
		// user switches, since mUserId will be used for the entire session.
		// Once created, keyguard should *never* re-use this instance with
		// another user.
		// In other words, mUserId should never change - hence it's marked
		// final.
		...
		try {
			...
			e.printStackTrace();
			// This should never happen, but it's better to have no widgets than
			// to crash.
			userContext = context;
		}

		mAppWidgetHost = new AppWidgetHost(userContext, APPWIDGET_HOST_ID,
		...
		mUserSetupCompleted = Settings.Secure
				.getIntForUser(mContext.getContentResolver(),
						Settings.Secure.USER_SETUP_COMPLETE, 0,
						UserHandle.USER_CURRENT) != 0;

		// Ensure we have the current state *before* we call
		...
		try {
			...
			Log.d(TAG, "lock screen instance created in keyguard "
					+ mLockScreenExt);
		} catch (Exception e) {
			Log.e(TAG, "exception: ", e);
		}
		
		//this.setOnTouchListener(tpTouchListener);
	...
	private void getInitialTransportState() {
		...
		mTransportState = (dcs.clearing ? TRANSPORT_GONE : (KeyguardUtils
				.isMusicPlaying(dcs.playbackState) ? TRANSPORT_VISIBLE
				: TRANSPORT_INVISIBLE));

		if (DEBUGXPORT)
			Log.v(TAG, "Initial transport state: " + mTransportState
					+ ", pbstate=" + dcs.playbackState);
	}
	...
	private void cleanupAppWidgetIds() {
		...
		// Clean up appWidgetIds that are bound to lockscreen, but not actually
		// used
		// This is only to clean up after another bug: we used to not call
		// deleteAppWidgetId when a user manually deleted a widget in keyguard.
		// This code
		// shouldn't have to run more than once per user. AppWidgetProviders
		// rely on callbacks
		// that are triggered by deleteAppWidgetId, which is why we're doing
		// this
		int[] appWidgetIdsInKeyguardSettings = mLockPatternUtils
				.getAppWidgets();
		...
		for (int i = 0; i < appWidgetIdsBoundToHost.length; i++) {
			...
			if (!contains(appWidgetIdsInKeyguardSettings, appWidgetId)) {
				Log.d(TAG,
						"Found a appWidgetId that's not being used by keyguard, deleting id "
								+ appWidgetId);
	...
	private KeyguardUpdateMonitorCallback mUpdateMonitorCallbacks = new KeyguardUpdateMonitorCallback() {
		...
		@Override
		void onMusicClientIdChanged(int clientGeneration, boolean clearing,
				android.app.PendingIntent intent) {
			// Set transport state to invisible until we know music is playing
			// (below)
			if (DEBUGXPORT
					&& (mClientGeneration != clientGeneration || clearing)) {
				Log.v(TAG, (clearing ? "hide" : "show") + " transport, gen:"
						+ clientGeneration);
			}
			mClientGeneration = clientGeneration;
			final int newState = (clearing ? TRANSPORT_GONE
					: (mTransportState == TRANSPORT_VISIBLE ? TRANSPORT_VISIBLE
							: TRANSPORT_INVISIBLE));
			if (newState != mTransportState) {
				mTransportState = newState;
				if (DEBUGXPORT)
					Log.v(TAG, "update widget: transport state changed");
				KeyguardHostView.this.post(mSwitchPageRunnable);
			}
		}

		@Override
		public void onMusicPlaybackStateChanged(int playbackState,
				long eventTime) {
			if (DEBUGXPORT)
				Log.v(TAG, "music state changed: " + playbackState);
			if (mTransportState != TRANSPORT_GONE) {
				final int newState = (KeyguardUtils
						.isMusicPlaying(playbackState) ? TRANSPORT_VISIBLE
						: TRANSPORT_INVISIBLE);
				if (newState != mTransportState) {
					mTransportState = newState;
					if (DEBUGXPORT)
						Log.v(TAG, "update widget: play state changed");
					KeyguardHostView.this.post(mSwitchPageRunnable);
				}
			}
		}
	...
	private int getWidgetPosition(int id) {
		...
		if (mAppWidgetContainer != null) {
			...
			for (int i = 0; i < children; i++) {
				final View content = appWidgetContainer.getWidgetPageAt(i)
						.getContent();
				if (content != null && content.getId() == id) {
					return i;
				} else if (content == null) {
					// Attempt to track down bug #8886916
					Log.w(TAG, "*** Null content at " + "i=" + i + ",id=" + id
							+ ",N=" + children);
				}
			}
			return -1;
	...
	protected void onFinishInflate() {
		...
		if (mSlidingChallengeLayout != null) {
			mSlidingChallengeLayout
					.setOnChallengeScrolledListener(mViewStateManager);
		}
		...
		mAppWidgetContainer.setBouncerAnimationDuration(challenge
		...
		if (KeyguardUpdateMonitor.getInstance(mContext).hasBootCompleted()) {
			...
			// We can't add widgets until after boot completes because
			// AppWidgetHost may try
	...
	private void updateAndAddWidgets() {
		...
		if (mSlidingChallengeLayout != null) {
			mSlidingChallengeLayout
					.setEnableChallengeDragging(!widgetsDisabled());
		}
	...
	private void setBackButtonEnabled(boolean enabled) {
		...
		setSystemUiVisibility(enabled ? getSystemUiVisibility()
				& ~View.STATUS_BAR_DISABLE_BACK : getSystemUiVisibility()
	...
	private int getDisabledFeatures(DevicePolicyManager dpm) {
		if (KeyguardUtils.isMediatekLCASupport()) {
			...
			if (DEBUG)
				KeyguardUtils.xlogD(TAG,
						"[LCA] Disable keyguard widget/camera.");
	...
	private boolean widgetsDisabled() {
		...
		boolean disabledByDpm = (mDisabledFeatures & DevicePolicyManager.KEYGUARD_DISABLE_WIDGETS_ALL) != 0;
	...
	protected void onAttachedToWindow() {
		...
		if (mLockScreenExt != null) {
			...
			mLockScreenExt.initDgilLayout(mSecurityModel.getSecurityMode()
					.ordinal(), SecurityMode.Pattern.ordinal(), this);
		}
	...
	private KeyguardWidgetPager.Callbacks mWidgetCallbacks = new KeyguardWidgetPager.Callbacks() {
		...
		public void onRemoveView(View v, boolean deletePermanently) {
			if (deletePermanently) {
				final int appWidgetId = ((KeyguardWidgetFrame) v)
	...
	public long getUserActivityTimeout() {
		...
		// Could also take into account longer timeouts for certain security
		// views.
		if (mAppWidgetContainer != null) {
	...
	private KeyguardSecurityCallback mCallback = new KeyguardSecurityCallback() {
		...
		public void dismiss(boolean authenticated) {
			...
			// /M: callback from lock views, not to use bouncer style for next
			// view
		...
		public void updateClipChildren(boolean clipChildren) {
			if (mSlidingChallengeLayout != null) {
				mSlidingChallengeLayout
						.setClipChildrenForNewEventView(clipChildren);
			}
		}
	...
	private void showAlmostAtWipeDialog(int attempts, int remaining) {
		String message = mContext
				.getString(R.string.kg_failed_attempts_almost_at_wipe,
						attempts, remaining);
		showDialog(null, message);
	}
	...
	private void showWipeDialog(int attempts) {
		String message = mContext.getString(
				R.string.kg_failed_attempts_now_wiping, attempts);
		showDialog(null, message);
	}
	...
	private void showAlmostAtAccountLoginDialog() {
		...
		String message = mContext.getString(
				R.string.kg_failed_attempts_almost_at_login, count,
				LockPatternUtils.FAILED_ATTEMPTS_BEFORE_TIMEOUT,
				timeoutInSeconds);
		showDialog(null, message);
	}
	...
	private void reportFailedUnlockAttempt() {
		final KeyguardUpdateMonitor monitor = KeyguardUpdateMonitor
				.getInstance(mContext);
		final int failedAttempts = monitor.getFailedUnlockAttempts() + 1; // +1
																			// for
																			// this
																			// time
		...
		final int failedAttemptsBeforeWipe = mLockPatternUtils
				.getDevicePolicyManager().getMaximumFailedPasswordsForWipe(
						null, mLockPatternUtils.getCurrentUser());
		...
		if (remainingBeforeWipe < LockPatternUtils.FAILED_ATTEMPTS_BEFORE_WIPE_GRACE) {
			...
			// DevicePolicyManager
			// that requests device wipe after N attempts. Once we get below the
			// grace
			// period, we'll post this dialog every time as a clear warning
			// until the
			...
			if (usingPattern && mEnableFallback) {
				if (failedAttempts == failedAttemptWarning) {
					...
					// don't show timeout dialog because we show account unlock
					// screen next
					showTimeout = false;
				}
			}
	...
	/**
	 * Shows the primary security screen for the user. This will be either the
	 * multi-selector or the user's security method.
	 * 
	 * @param turningOff
	...
	void showPrimarySecurityScreen(boolean turningOff) {
		if (DEBUG)
			KeyguardUtils.xlogD(TAG, "showPrimarySecurityScreen(turningOff="
					+ turningOff + ") enter");
		SecurityMode securityMode = mSecurityModel.getSecurityMode();
		if (DEBUG)
			KeyguardUtils.xlogD(TAG,
					"showPrimarySecurityScreen query securityMode finish securityMode="
							+ securityMode);

		if (KeyguardUpdateMonitor.getInstance(mContext)
				.isAlternateUnlockEnabled()) {
			SecurityMode alternateMode = mSecurityModel
					.getAlternateFor(securityMode);
			if (DEBUG)
				KeyguardUtils.xlogD(TAG,
						"showPrimarySecurityScreen alternateMode = "
								+ alternateMode);
			if (!turningOff) {
		...
		if (DEBUG)
			KeyguardUtils.xlogD(TAG,
					"showPrimarySecurityScreen getAlternateFor finish");

		if (isCurrentSimPinPukView()
				&& (securityMode == SecurityMode.SimPinPukMe1
						|| securityMode == SecurityMode.SimPinPukMe2
						|| securityMode == SecurityMode.SimPinPukMe3 || securityMode == SecurityMode.SimPinPukMe4)) {
			Log.d(TAG,
					"Skip showSecurityScreen() while currently SimPinPukViews is showing");
			return;
		}
		...
		if (mSlidingChallengeLayout != null) {
			...
			if (mCurrentKeyguardSecurityView instanceof KeyguardSimPinPukView
				...
				final int smallWidgetMinHeight = res
						.getDimensionPixelSize(R.dimen.keyguard_small_widget_height_for_phone);
				if (mSlidingChallengeLayout.getMaxChallengeTop() > 0
					...
					// / M: if widget can't be completely shown, always show
					// statusbar time
	...
	/**
	 * Shows the backup security screen for the current security mode. This
	 * could be used for password recovery screens but is currently only used
	 * for pattern unlock to show the account unlock screen and biometric unlock
	 * to show the user's normal unlock.
	 */
	private void showBackupSecurityScreen() {
		if (DEBUG)
			Log.d(TAG, "showBackupSecurity()");
		SecurityMode backup = mSecurityModel
				.getBackupSecurityMode(mCurrentSecuritySelection);
	...
	public boolean showNextSecurityScreenIfPresent() {
		...
		if (SecurityMode.None == securityMode) {
			...
			showSecurityScreen(securityMode); // switch to the alternate
												// security view
			return true;
		}
	}

	// / M: ALPS00513027 for gemini SIM pin unlock, not to encter bouncer mode
	// in 2nd simPinPuk view
	private void showNextSecurityScreenOrFinish(boolean authenticated,
		...
		if (SecurityMode.None == mCurrentSecuritySelection) {
			...
			case SimPinPukMe3:
			case SimPinPukMe4:
				// Shortcut for SIM PIN/PUK to go to directly to user's security
				// screen or home
				SecurityMode securityMode = mSecurityModel.getSecurityMode();
			...
			case AntiTheft:
				...
				break;
			default:
				Log.v(TAG, "Bad security screen " + mCurrentSecuritySelection
						+ ", fail safe");
				showPrimarySecurityScreen(false);
				break;
			}
			...
			if (mLockPatternUtils.isLockScreenDisabled()
					&& mSecurityModel.getSecurityMode() == SecurityMode.None) {
				// / M: ALPS01370779 if entering here, it means that the
				// security view is dismissed without pw.
				// / We need to choose to show next screen or just finish.
				// / If keyguard is disabled in Settings, just go to finish
				// everything.
				Log.d(TAG,
						"Skip showPrimarySecurityScreen() while keyguard is disabled in settings.");
				finish = true;
			} else {
				showPrimarySecurityScreen(false);
				bShowBouncer = true;
		...
		if (finish) {
			...
			// If there's a pending runnable because the user interacted with a
			// widget
			...
			// M: Improving user experience to show message in next
			// SimPinPukView
			if (bShowBouncer) {
		...
		public boolean onClickHandler(final View view,
			...
			if (pendingIntent.isActivity()) {
				...
				if (hostView.mViewStateManager.isChallengeShowing()) {
					...
					// / M: directly call dismiss method to show bouncer style
					// for next lock view
					hostView.dismiss();
				}
				return true;
	...
	// Used to ignore callbacks from methods that are no longer current (e.g.
	// face unlock).
	...
	private KeyguardSecurityView getSecurityView(SecurityMode securityMode) {
		...
		// /M: mediatek add only for PIN1 and PIN2, in this case, we needn't
		// recreate from the same layout file
		...
		if (view instanceof KeyguardSelectorView) {
			...
			View carrierText = selectorView
					.findViewById(R.id.keyguard_selector_fade_container);
	...
	/**
	 * Switches to the given security view unless it's already being shown, in
	...
	private void showSecurityScreen(SecurityMode securityMode) {
		...
		if (DEBUG)
			KeyguardUtils.xlogD(TAG, "showSecurityScreen get oldView");
		KeyguardSecurityView newView = getSecurityView(securityMode);
		if (DEBUG)
			KeyguardUtils.xlogD(TAG, "showSecurityScreen get newView");
		...
		boolean isSimOrAccount = securityMode == SecurityMode.Account
				...
				|| securityMode == SecurityMode.SimPinPukMe4;
		mAppWidgetContainer
				.setVisibility(((isSimOrAccount && fullScreenEnabled) || PowerOffAlarmManager
						.isAlarmBoot()) ? View.GONE : View.VISIBLE);
		// / @}

		// Don't show camera or search in navbar when SIM or Account screen is
		// showing
		setSystemUiVisibility(isSimOrAccount ? (getSystemUiVisibility() | View.STATUS_BAR_DISABLE_SEARCH)
				: (getSystemUiVisibility() & ~View.STATUS_BAR_DISABLE_SEARCH));

		if (mSlidingChallengeLayout != null) {
			// / M: If dm lock is on or not AccountView, we should also set
			// changelayout interactive
			mSlidingChallengeLayout
					.setChallengeInteractive((securityMode != SecurityMode.Account)
							|| AntiTheftManager.isAntiTheftLocked());
		}

		if (DEBUG)
			KeyguardUtils.xlogD(TAG, "showSecurityScreen pause oldView before");

		// Emulate Activity life cycle
		if (oldView != null) {
			oldView.onPause();
			oldView.setKeyguardCallback(mNullCallback); // ignore requests from
														// old view

			// / M: If KeyguardSelectorView is to be phase out, also invisible
			// keyguard layer @{
			if (oldView instanceof KeyguardSelectorView
					&& !(newView instanceof KeyguardSelectorView)
					&& mSlidingChallengeLayout != null) {
				mKeyaugerLayerVisibility = false;
				updateKeyguardLayerVisibility();
				// / M: KeyguardSelectorView's NewEventView may draw out of its
				// region, if it hides, we reset clip request to true
				requestClipChildren(true);
			} else if (oldView instanceof KeyguardSelectorView
					&& !(newView instanceof KeyguardSelectorView)
					&& mMultiPaneChallengeLayout != null) {
				mKeyaugerLayerVisibility = false;
				updateKeyguardLayerVisibility();
		...
		if (DEBUG)
			KeyguardUtils.xlogD(TAG, "showSecurityScreen pause oldView finish");
		...
		// @{
		if (newView instanceof KeyguardSelectorView
				&& mSlidingChallengeLayout != null) {
			mKeyaugerLayerVisibility = true;
			updateKeyguardLayerVisibility();
			KeyguardSelectorView keyguardSelectorView = (KeyguardSelectorView) newView;
			mSlidingChallengeLayout
					.setTouchRecepientForKeyguardLayer(keyguardSelectorView
							.getGlowPadView());
			// / M: KeyguardSelectorView's NewEventView may draw out of its
			// region, if it shows, we disable clip request, or if user
			// / drag NewEventView out of KeyguardSelecotor's frame, it cannot
			// show
			requestClipChildren(false);
		} else if (newView instanceof KeyguardSelectorView
				&& mMultiPaneChallengeLayout != null) {
			mKeyaugerLayerVisibility = true;
			updateKeyguardLayerVisibility();
			KeyguardSelectorView keyguardSelectorView = (KeyguardSelectorView) newView;
			...
			// / @}
		if (DEBUG)
			Log.d(TAG, "showSecurityScreen update keyguard layer finish");
		...
		if (securityMode == SecurityMode.None) {
			// Discard current runnable if we're switching back to the selector
			// view
			setOnDismissAction(null);
		}
		if (securityMode == SecurityMode.Account
				&& !mLockPatternUtils.isPermanentlyLocked()) {
			// we're showing account as a backup, provide a way to get back to
			// primary
			setBackButtonEnabled(true);
		}
		mCurrentSecuritySelection = securityMode;
		// / M: Save current security view, we must release the KeyguardCallback
		// set before
		mCurrentKeyguardSecurityView = newView;

		if (DEBUG)
			KeyguardUtils.xlogD(TAG, "showSecurityScreen(" + securityMode
					+ ") finish");
	}
	...
	public void onScreenTurnedOn() {
		...
		// /M: After screen turned on, reset the widget to the appriotate
		// widget, and popup the unlock view. @{
		mSwitchPageRunnable.run();
		// / M @}
	...
	public void onScreenTurnedOff() {
		if (DEBUG)
			Log.d(TAG,
					String.format("screen off, instance %s at %s",
		...
		// / M @}
		// Once the screen turns off, we no longer consider this to be first
		// boot and we want the
		// biometric unlock to start next time keyguard is shown.
		KeyguardUpdateMonitor.getInstance(mContext).setAlternateUnlockEnabled(
				true);
		// We use mAppWidgetToShow to show a particular widget after you add
		// it-- once the screen
		...
		// /M: After screen turned off, reset the widget to the appriotate
		// widget, and popup the unlock view. @{
		mSwitchPageRunnable.run();
		// / M @}
	...
	private int getSecurityViewIdForMode(SecurityMode securityMode) {
		...
		case SimPinPukMe1:
			return R.id.keyguard_sim_pin_puk_view;
		case SimPinPukMe2:
			return R.id.keyguard_sim_pin_puk_view;
		case SimPinPukMe3:
			return R.id.keyguard_sim_pin_puk_view;
	...
	private int getLayoutIdFor(SecurityMode securityMode) {
		...
		case SimPinPukMe1:
			return R.layout.mtk_keyguard_sim_pin_puk_view;
		case SimPinPukMe2:
			return R.layout.mtk_keyguard_sim_pin_puk_view;
		case SimPinPukMe3:
			return R.layout.mtk_keyguard_sim_pin_puk_view;
	...
	private boolean addWidget(int appId, int pageIndex, boolean updateDbIfFailed) {
		...
		if (appWidgetInfo != null) {
			AppWidgetHostView view = mAppWidgetHost.createView(mContext, appId,
					appWidgetInfo);
			addWidget(view, pageIndex);
			return true;
		} else {
			if (updateDbIfFailed) {
				Log.w(TAG, "*** AppWidgetInfo for app widget id " + appId
						+ "  was null for user" + mUserId + ", deleting");
	...
	private final CameraWidgetFrame.Callbacks mCameraWidgetCallbacks = new CameraWidgetFrame.Callbacks() {
		@Override
		...
		public void onCameraLaunchedSuccessfully() {
			if (mAppWidgetContainer.isCameraPage(mAppWidgetContainer
					.getCurrentPage())) {
	...
	private void addDefaultWidgets() {
		if (!mSafeModeEnabled && !widgetsDisabled()) {
			...
			View addWidget = inflater.inflate(R.layout.keyguard_add_widget,
					this, false);
			mAppWidgetContainer.addWidget(addWidget, 0);
			View addWidgetButton = addWidget
					.findViewById(R.id.keyguard_add_widget_view);
			addWidgetButton.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					// Pass in an invalid widget id... the picker will allocate
					// an ID for us
					mActivityLauncher
		...
		// We currently disable cameras in safe mode because we support loading
		// 3rd party
		// cameras we can't trust. TODO: plumb safe mode into camera creation
		// code and only
		...
		if (!mSafeModeEnabled
				...
				&& mContext.getResources().getBoolean(
						R.bool.kg_enable_camera_default_widget)) {
			View cameraWidget = CameraWidgetFrame.create(mContext,
					mCameraWidgetCallbacks, mActivityLauncher);
	...
	private KeyguardTransportControlView getOrCreateTransportControl() {
		if (mTransportControl == null) {
			LayoutInflater inflater = LayoutInflater.from(mContext);
			mTransportControl = (KeyguardTransportControlView) inflater
					.inflate(R.layout.keyguard_transport_control_view, this,
							false);
			mTransportControl
					.setTransportControlCallback(new TransportControlCallback() {
						public void userActivity() {
							mViewMediatorCallback.userActivity();
						}
					});
	...
	private int getInsertPageIndex() {
		View addWidget = mAppWidgetContainer
				.findViewById(R.id.keyguard_add_widget);
		...
		if (widgets == null) {
			...
			for (int i = widgets.length - 1; i >= 0; i--) {
				if (widgets[i] == LockPatternUtils.ID_DEFAULT_STATUS_WIDGET) {
					...
					// We add the widgets from left to right, starting after the
					// first page after
					// the add page. We count down, since the order will be
					// persisted from right
		...
		if (!widgetPageExists) {
			...
			if (!mSafeModeEnabled) {
				if (userAddedWidgetsEnabled) {
					...
					if (appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID) {
						addedDefaultAppWidget = addWidget(appWidgetId,
								insertPageIndex, true);
					}
				} else {
					// note: even if widgetsDisabledByDpm() returns true, we
					// still bind/create
					// the default appwidget if possible
					int appWidgetId = mLockPatternUtils
							.getFallbackAppWidgetId();
					if (appWidgetId == AppWidgetManager.INVALID_APPWIDGET_ID) {
						appWidgetId = allocateIdForDefaultAppWidget();
						if (appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID) {
							mLockPatternUtils
									.writeFallbackAppWidgetId(appWidgetId);
						}
					}
					if (appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID) {
						addedDefaultAppWidget = addWidget(appWidgetId,
								insertPageIndex, false);
						if (!addedDefaultAppWidget) {
							mAppWidgetHost.deleteAppWidgetId(appWidgetId);
							mLockPatternUtils
									.writeFallbackAppWidgetId(AppWidgetManager.INVALID_APPWIDGET_ID);
						}
					}
				}
			}

			// Use the built-in status/clock view if we can't inflate the
			// default widget
			if (!addedDefaultAppWidget) {
			...
			if (!mSafeModeEnabled && userAddedWidgetsEnabled) {
				mAppWidgetContainer.onAddView(
						mAppWidgetContainer.getChildAt(insertPageIndex),
						insertPageIndex);
			}
		}
	}
	...
	public Parcelable onSaveInstanceState() {
		if (DEBUG)
			Log.d(TAG, "onSaveInstanceState, tstate=" + mTransportState);
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		setInsets(ss.insets);
		if (DEBUG)
			Log.d(TAG, "onRestoreInstanceState, transport=" + mTransportState);
		mSwitchPageRunnable.run();
	}
	...
	private void setInsets(Rect insets) {
		mInsets.set(insets);
		if (mSlidingChallengeLayout != null)
			mSlidingChallengeLayout.setInsets(mInsets);
		if (mMultiPaneChallengeLayout != null)
			mMultiPaneChallengeLayout.setInsets(mInsets);

		final CameraWidgetFrame cameraWidget = findCameraPage();
		if (cameraWidget != null)
			cameraWidget.setInsets(mInsets);
	}
	...
	private void showAppropriateWidgetPage() {
		...
		if (!transportAdded) {
			...
			// If the transport was just added, we need to wait for layout to
			// happen before
	...
	/**
	 * Examines the current state and adds the transport to the widget pager
	 * when the state changes.
	 * 
	 * Showing the initial transport and keeping it around is a bit tricky
	 * because the signals coming from music players aren't always clear. Here's
	 * how the states are handled:
	 * 
	 * {@link TRANSPORT_GONE} means we have no reason to show the transport -
	 * remove it if present.
	 * 
	 * {@link TRANSPORT_INVISIBLE} means we have potential to show the transport
	 * because a music player is registered but not currently playing music (or
	 * we don't know the state yet). The code adds it conditionally on play
	 * state.
	 * 
	 * {@link #TRANSPORT_VISIBLE} means a music player is active and transport
	 * should be showing.
	 * 
	 * Once the transport is showing, we always show it until keyguard is
	 * dismissed. This state is maintained by onSave/RestoreInstanceState().
	 * This state is cleared in {@link KeyguardViewManager#hide} when keyguard
	 * is dismissed, which causes the transport to be gone when keyguard is
	 * restarted until we get an update with the current state.
	 * 
	 * @param state
	 */
	private boolean ensureTransportPresentOrRemoved(int state) {
		...
		final boolean shouldBeVisible = state == TRANSPORT_INVISIBLE
				&& KeyguardUtils.isMusicPlaying(state);
		if (!showing && (visible || shouldBeVisible)) {
			// insert to left of camera if it exists, otherwise after right-most
			// widget
			int lastWidget = mAppWidgetContainer.getChildCount() - 1;
			int position = 0; // handle no widget case
			if (lastWidget >= 0) {
				position = mAppWidgetContainer.isCameraPage(lastWidget) ? lastWidget
						: lastWidget + 1;
			}
			if (DEBUGXPORT)
				Log.v(TAG, "add transport at " + position);
			mAppWidgetContainer.addWidget(getOrCreateTransportControl(),
					position);
			return true;
	...
	boolean isMusicPage(int pageIndex) {
		return pageIndex >= 0
				&& pageIndex == getWidgetPosition(R.id.keyguard_transport_control);
	}
	...
	private int getAppropriateWidgetPage(int musicTransportState) {
		...
		if (mAppWidgetToShow != AppWidgetManager.INVALID_APPWIDGET_ID) {
			final int childCount = mAppWidgetContainer.getChildCount();
			for (int i = 0; i < childCount; i++) {
				if (mAppWidgetContainer.getWidgetPageAt(i)
						.getContentAppWidgetId() == mAppWidgetToShow) {
					return i;
				}
			}
			mAppWidgetToShow = AppWidgetManager.INVALID_APPWIDGET_ID;
		...
		if (musicTransportState == TRANSPORT_VISIBLE) {
			...
			return mAppWidgetContainer
					.getWidgetPageIndex(getOrCreateTransportControl());
		}
			...
			if (multiUserView instanceof KeyguardMultiUserSelectorView) {
				...
				UserSwitcherCallback callback = new UserSwitcherCallback() {
					...
					public void hideSecurityView(int duration) {
						mSecurityViewContainer.animate().alpha(0)
	...
	public void cleanUp() {
		// Make sure we let go of all widgets and their package contexts
		// promptly. If we don't do
		// this, and the associated application is uninstalled, it can cause a
		// soft reboot.
	...
	/**
	 * In general, we enable unlocking the insecure keyguard with the menu key.
	...
	public boolean handleBackKey() {
		...
		if (mCurrentSecuritySelection != SecurityMode.None) {
			// / M: directly call dismiss method to show bouncer style for next
			// lock view
			dismiss();
			return true;
		}
	 ...
	 */
	public void dismiss() {
		// / M: If antitheft lock is on and cannot be dismissed, should not let
		// user by pass lockscreen
		if (!AntiTheftManager.isDismissable()) {
			return;
		}
	...
	public void showAssistant() {
		final Intent intent = ((SearchManager) mContext
		...
		final ActivityOptions opts = ActivityOptions.makeCustomAnimation(
				mContext, R.anim.keyguard_action_assist_enter,
				R.anim.keyguard_action_assist_exit, getHandler(), null);

		intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);

		mActivityLauncher.launchActivityWithAnimation(intent, false,
				opts.toBundle(), null, null);
	}
	...
	void updateKeyguardLayerVisibility() {
		if (mSlidingChallengeLayout != null) {
			mSlidingChallengeLayout
					.updateKeyguardLayerVisibility(mKeyaugerLayerVisibility);
	...
	OnTouchListener tpTouchListener = new OnTouchListener() {
		
		@Override
		public boolean onTouch(View arg0, MotionEvent arg1) {
			// TODO Auto-generated method stub
			android.util.Log.e("leihai","touch");
			return false;
		}
	};
*/
