...
public final class ViewRootImpl implements ViewParent,
    ...
    private static boolean DEBUG_INPUT_PROCESSING = false || LOCAL_LOGV;
    ...
    /// M: used to check whether app launch time logs should be enabled dynamically.
    private static boolean sAppLaunchTimeLog = false;
    ...
    public ViewRootImpl(Context context, Display display) {
        ...
        sAppLaunchTimeLog = checkAppLaunchTimeProperty();
    }
    ...
    private void enableHardwareAcceleration(WindowManager.LayoutParams attrs) {
        ...
        if (DEBUG_DISABLEHW) {
            Xlog.d(TAG, "disable hardware acceleration by DEBUG!!!, this = " + this);
            return;
        }
        ...
        if (!hardwareAccelerated){
            if (DEBUG_DEFAULT) {
                Xlog.d(TAG, "disable hardware acceleration by APP!!!, this = " + this);
            }
        }
    ...
    public void requestLayout() {
        if (!mHandlingLayoutInLayoutRequest) {
            ...
            if (DEBUG_REQUESTLAYOUT) {
                Xlog.d(TAG, "requestLayout: mView = " + mView + ", this = " + this ,new Throwable("requestLayout"));
            }
            checkThread();
    ...
    public ViewParent invalidateChildInParent(int[] location, Rect dirty) {
        ...
        if (DEBUG_INVALIDATE) {
            Log.v(TAG, "Invalidate child: " + dirty + ",this = " + this, new Throwable("invalidateChildInParent"));
        }
        ...
        if (mFirst || windowShouldResize || insetsChanged ||
            ...
            try {
                ...
                if (DEBUG_LAYOUT) Log.v(TAG, "relayout: frame=" + frame.toShortString()
                        ...
                        + " surface=" + mSurface
                        + ",valid = " + mSurface.isValid() + ",surfaceGenerationId = "
                ...
                if (contentInsetsChanged) {
                    if (mWidth > 0 && mHeight > 0 && lp != null &&
                        ...
                        try {
                            ...
                            /// M : displayList may need refresh, try to getDisplayList first
                            mView.getDisplayList();
    ...
    private boolean ensureTouchModeLocally(boolean inTouchMode) {
        ...
        if (DEBUG_TOUCHMODE) {
            Xlog.d("touchmode", "ensureTouchModeLocally(" + inTouchMode + "), current " + "touch mode is "
                    + mAttachInfo.mInTouchMode + ", this = " + this, new Throwable("ensureTouchModeLocally"));
        }
    ...
    final class ImeInputStage extends AsyncInputStage
        ...
        public void onFinishedInputEvent(Object token, boolean handled) {
            QueuedInputEvent q = (QueuedInputEvent)token;
            if (DEBUG_IMF || DEBUG_INPUT || DEBUG_KEY) {
                Xlog.d(TAG, "IME finishedEvent: handled = " + handled + ", event = " + q + ",viewAncestor = " + this);
            }
            if (handled) {
                finish(q, true);
                return;
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        private int processKeyEvent(QueuedInputEvent q) {
            ...
            if (mView.dispatchKeyEvent(event)) {
                if (DEBUG_DEFAULT) {
                    Xlog.v(TAG, "App handle key event: event = " + event + ", mView = " + mView + ", this = " + this);
                }
                return FINISH_HANDLED;
            }
        ...
        private int processPointerEvent(QueuedInputEvent q) {
            ...
            if (mView.dispatchPointerEvent(event)) {
                if (DEBUG_DEFAULT) {
                    Xlog.v(TAG, "App handle pointer event: event = " + event + ", mView = " + mView + ", this = " + this);
                }
                return FINISH_HANDLED;
            }
            return FORWARD;
    ...
    private void scheduleProcessInputEvents() {
        if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
            Xlog.v(TAG, "scheduleProcessInputEvents: mPendingInputEventHead = "
    ...
    void doProcessInputEvents() {
        ...
        while (mPendingInputEventHead != null) {
            ...
            /** @} */
            if (DEBUG_INPUT || DEBUG_KEY || DEBUG_MOTION) {
                Xlog.v(TAG, "doProcessInputEvents: mCurrentInputEvent = " + q + ", this = " + this);
            }
            deliverInputEvent(q);
        }
...
//        DBG = enable;
    ...
    private static void checkViewRootImplLogProperty() {
        ...
        if (propString != null && propString.length() > 0) {
            ...
            DEBUG_REQUESTLAYOUT = (logFilter & DEBUG_REQUESTLAYOUT_FLAG) == DEBUG_REQUESTLAYOUT_FLAG;
            DEBUG_INVALIDATE = (logFilter & DEBUG_INVALIDATE_FLAG) == DEBUG_INVALIDATE_FLAG;
            DEBUG_SCHEDULETRAVERSALS = (logFilter & DEBUG_SCHEDULETRAVERSALS_FLAG) == DEBUG_SCHEDULETRAVERSALS_FLAG;
    ...
    private static boolean checkAppLaunchTimeProperty() {
        return (LOG_ENABLED == SystemProperties.getInt("persist.applaunchtime.enable", LOG_DISABLED));
    }
    ...
    public void dumpInputDispatchingStatus() {
        ...
        if (mCurrentKeyEvent == null) {
            Xlog.i(TAG, "ANR Key Analyze: No Key event currently.");
            Xlog.i(TAG, "ANR Key Analyze: Previeous Event " + mPreviousKeyEvent +  ",finish at " + simpleDateFormat.format(date2));
        } else {
            Xlog.i(TAG, "Dispatch " + mCurrentKeyEvent + " status is " + mKeyEventStatus + ",start at "
                    + simpleDateFormat.format(date) + ", spent " + dispatchTime + "ms.");
        }

        if (mCurrentMotion == null) {
            date2.setTime(mPreviousMotionEventFinishTime);
            Xlog.i(TAG, "ANR Motion Analyze: No motion event currently.");
            Xlog.i(TAG, "ANR Motion Analyze: Previeous Event " + mPreviousMotion +  ",finish at " + simpleDateFormat.format(date2));
        } else {
            ...
            Xlog.i(TAG, "Dispatch " + mCurrentMotion + " status is " + mMotionEventStatus
                    + ",start at " + simpleDateFormat.format(date) + ", spent " + dispatchTime + "ms.");
        }
