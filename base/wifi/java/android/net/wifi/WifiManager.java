...
public class WifiManager {
   ...
   /**
    ...
    /** @hide */
    public static final int SET_WIFI_DISCONNECT             = BASE + 59;
    /** @} */
    ...
    /**
     ...
     public boolean setPoorLinkThreshold(boolean isGood, double threshold) {
          try {
              return mService.setPoorLinkThreshold(isGood, threshold);
          } catch (RemoteException e) {
              return false;
          }
      }
    ...
    public int addHsCredential(String type, String username, String passwd, String imsi, String root_ca, String realm, String fqdn, String client_ca, String milenage, String simslot, String priority, String roamingconsortium, String mcc_mnc) {
        Log.d(TAG, "addHsCredential, type = " + type + " username = " + username + " passwd = " + passwd + " imsi = " + imsi + " root_ca = " + root_ca + " realm = " + realm + " fqdn = " + fqdn + " client_ca = " + client_ca + " milenage = " + milenage + " simslot = " + simslot + " priority = " + priority + " roamingconsortium = " + roamingconsortium + " mcc_mnc = " + mcc_mnc);
        ...
        try {
            return mService.addHsCredential(type, username, passwd, imsi, root_ca, realm, fqdn, client_ca, milenage, simslot, priority, roamingconsortium, mcc_mnc);
        } catch (RemoteException e) {
            return -1;
        }
    }
    ...
    /**
     *  for Hotknot, enable wifi for p2p, sta mode should not connect
     * @return {@code true} if the operation succeeds, {@code false} otherwise
     * @hide
     */
    public boolean setWifiEnabledDefaultDisconnected(boolean enabled) {
        Log.d(TAG, "setWifiEnabledDefaultDisconnected,"+enabled);

        validateChannel();
        if(enabled==true){
            sAsyncChannel.sendMessage(SET_WIFI_DISCONNECT, 1, putListener(null));
        }else{
            sAsyncChannel.sendMessage(SET_WIFI_DISCONNECT, 0, putListener(null));
        }
        try {
            return mService.setWifiEnabled(enabled);
        } catch (RemoteException e) {
