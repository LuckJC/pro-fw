...
void Layer::clearTexture() {
    LAYER_LOGD("Layer %p clearTexture %d", this, texture.id);
    texture.id = 0;
    /// M: force to clear bound texture    
    caches.bindTexture(renderTarget, texture.id);
}
...
void Layer::flush() {
    ...
    if (deferredList && renderer) {
        if (displayList) {
            TT_UPDATE(texture.id, false, String8(displayList->getName()));
        } else {
            ALOGE("flush without displayList %d", texture.id);
        }
        ...
        DUMP_LAYER(layer.getWidth(), layer.getHeight(), getFbo());
