...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    private IMtkHdmiManager mHdmiManager = null; // add for SMB
    ///@}
    ...
    private boolean mSBRDMALimited = false; // for SMB RDMA Limitation
    ...
    public void systemReady(TwilightService twilight, DreamManagerService dreamManager) {
        synchronized (mLock) {
            ...
            {
                try {
                    if (mHdmiManager.hasCapability(IHDMINative.CAPABILITY_NO_CALL)) {
                        Slog.d(TAG, "RDMA with limitation");
                        mSBRDMALimited = true;
                    } else {
                        Slog.d(TAG, "RDMA without limitation");
                        mSBRDMALimited = false;
                    }
    ...
    private final class SmartBookPlugReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
                ...
                mDisplayPowerController.sbCheckAndDismissElectron();
                ...
                if (mSbPlugState != sbPlugState) {
                    if (sbPlugState) {
                        ...
                        /* Disable AAL when smart book plug in */
                        if (DEBUG_SPEW) {
                            Slog.d(TAG, "smart book mode: disable AAL when smart book plug in");
                        }
                        mDisplayPowerController.setSbAALEnabled(false);
                        ...
                        /* Enable AAL when smart book plug in */
                        if (DEBUG_SPEW) {
                            Slog.d(TAG, "smart book mode: enable AAL when smart book plug out");
                        }
                        mDisplayPowerController.setSbAALEnabled(true);
                    }
