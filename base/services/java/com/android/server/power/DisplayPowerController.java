...
final class DisplayPowerController {
    ...
    // [Smart Book] Proximity sensor debounce message
    private static final int MSG_SB_PROXIMITY_SENSOR_DEBOUNCED = 4;
    ...
    private static final long BRIGHTENING_LIGHT_DEBOUNCE = 4000; /// MTK: Auto backlight
    private static final long DARKENING_LIGHT_DEBOUNCE = 4000;   /// MTK: Auto backlight

    // Hysteresis constraints for brightening or darkening.
    // The recent lux must have changed by at least this fraction relative to the
    // current ambient lux before a change will be considered.
    private static final float BRIGHTENING_LIGHT_HYSTERESIS = 0.20f; /// MTK: Auto backlight
    private static final float DARKENING_LIGHT_HYSTERESIS = 0.40f; /// MTK: Auto backlight
    ...
    // [Smart Book] Set to true if the proximity sensor listener has been registered
    // with the sensor manager.
    private boolean mSbProximitySensorEnabled;

    // The debounced proximity sensor state.
    private int mProximity = PROXIMITY_UNKNOWN;

    // [Smart Book] The debounced proximity sensor state.
    private int mSbProximity = PROXIMITY_UNKNOWN;

    // The raw non-debounced proximity sensor state.
    private int mPendingProximity = PROXIMITY_UNKNOWN;
    private long mPendingProximityDebounceTime = -1; // -1 if fully debounced

    // [Smart Book] The raw non-debounced proximity sensor state.
    private int mSbPendingProximity = PROXIMITY_UNKNOWN;
    private long mSbPendingProximityDebounceTime;

    // True if the screen was turned off because of the proximity sensor.
    // When the screen turns on again, we report user activity to the power manager.
    private boolean mScreenOffBecauseOfProximity;

    // [Smart Book] True if the screen was turned off because of the proximity sensor.
    // When the screen turns on again, we report user activity to the power manager.
    private boolean mSbScreenOffBecauseOfProximity;

    // [Smart Book] True if the screen on animation is triggered
    private boolean mSbScreenOnIsStart = false;
    ...
    /**
     * [Smart Book] Disable/Enable AAL Feature
    */
    public void setSbAALEnabled(boolean enable) {
        int target;
    ...
    /**
     * [Smart Book] Enable/disable Proximity sensor
    */
    public void setSbProximitySensorEnabled(boolean enable) {
        if (enable) {
            if (!mSbProximitySensorEnabled) {
                mSbProximitySensorEnabled = true;
                mSbPendingProximity = PROXIMITY_UNKNOWN;
                mSensorManager.registerListener(mSbProximitySensorListener, mProximitySensor,
                        SensorManager.SENSOR_DELAY_NORMAL, mHandler);
            }
        } else {
            if (mSbProximitySensorEnabled) {
                mSbProximitySensorEnabled = false;
                mSbProximity = PROXIMITY_UNKNOWN;
                mHandler.removeMessages(MSG_SB_PROXIMITY_SENSOR_DEBOUNCED);
                mSensorManager.unregisterListener(mSbProximitySensorListener);
            }
        }
    }

    /**
     * [Smart Book] Proximity sensor listener
    */
    private final SensorEventListener mSbProximitySensorListener = new SensorEventListener() {
        @Override
        public void onSensorChanged(SensorEvent event) {
            if (mSbProximitySensorEnabled) {
                ...
                if (DEBUG) {
                    Slog.d(TAG, "SB P-Sensor Changed: " + positive);
                }
                handleSbProximitySensorEvent(time, positive);
            }
        }
    ...
    /**
     * [Smart Book] Proximity sensor event handler
    */
    private void handleSbProximitySensorEvent(long time, boolean positive) {
        if (mSbPendingProximity == PROXIMITY_NEGATIVE && !positive) {
            return; // no change
        }
        if (mSbPendingProximity == PROXIMITY_POSITIVE && positive) {
            return; // no change
        }
        ...
        mHandler.removeMessages(MSG_SB_PROXIMITY_SENSOR_DEBOUNCED);
        if (positive) {
            mSbPendingProximity = PROXIMITY_POSITIVE;
            mSbPendingProximityDebounceTime = time + PROXIMITY_SENSOR_POSITIVE_DEBOUNCE_DELAY;
        } else {
            mSbPendingProximity = PROXIMITY_NEGATIVE;
            mSbPendingProximityDebounceTime = time + PROXIMITY_SENSOR_NEGATIVE_DEBOUNCE_DELAY;
        }
        if (DEBUG) {
            Slog.d(TAG, "mSbPendingProximity=" + mSbPendingProximity);
        }
        debounceSbProximitySensor();
    }

    /**
     * [Smart Book] Proximity sensor debounce
    */
    private void debounceSbProximitySensor() {
        if (mSbPendingProximity != PROXIMITY_UNKNOWN) {
            final long now = SystemClock.uptimeMillis();
            if (mSbPendingProximityDebounceTime <= now) {
                mSbProximity = mSbPendingProximity;
                // Apply the proximity sensor.
                if (mProximitySensor != null) {
                    if (DEBUG) {
                        Slog.d(TAG, "mSbProximity = " + mSbProximity);
                    }
                    if (!mSbScreenOffBecauseOfProximity
                            && mSbProximity == PROXIMITY_POSITIVE) {
                        mSbScreenOffBecauseOfProximity = true;
                        sendOnSbProximityPositive();
                        if (DEBUG) {
                            Slog.d(TAG, "Turn Off screen becaue of P-sensor closing in smart book mode.");
                        }
                    } else if (mSbScreenOffBecauseOfProximity && mSbProximity == PROXIMITY_NEGATIVE) {
                        mSbScreenOffBecauseOfProximity = false;
                        sendOnSbProximityNegative();
                        if (DEBUG) {
                            Slog.d(TAG, "Turn On screen becaue of P-sensor away in smart book mode.");
                        }
                    }
                }
            } else {
                Message msg = mHandler.obtainMessage(MSG_SB_PROXIMITY_SENSOR_DEBOUNCED);
                msg.setAsynchronous(true);
                mHandler.sendMessageAtTime(msg, mSbPendingProximityDebounceTime);
            }
        }
    }

    /**
     * [Smart Book] Proximity sensor positive event
    */
    private void sendOnSbProximityPositive() {
        mCallbackHandler.post(mOnSbProximityPositiveRunnable);
    }

    private final Runnable mOnSbProximityPositiveRunnable = new Runnable() {
        @Override
        public void run() {
            mCallbacks.onSbProximityPositive();
        }
    };

    /**
     * [Smart Book] Proximity sensor negative event
    */
    private void sendOnSbProximityNegative() {
        mCallbackHandler.post(mOnSbProximityNegativeRunnable);
    }

    private final Runnable mOnSbProximityNegativeRunnable = new Runnable() {
        @Override
        public void run() {
            mCallbacks.onSbProximityNegative();
        }
    };
    ...
    public void sbCheckAndDismissElectron() {
        if (mSbScreenOnIsStart) {
            Slog.d(TAG, "Smartbook plug in but Electron not finished, dismissElectronBeam");
    ...
    private void updatePowerState() {
        ...
        if (mDisplayManager.isSmartBookPluggedIn()) {
            ...
            Slog.d(TAG, "Smartbook plug in, bypass updatePowerState in DisplayPowerController");
            return;
        }
        ...
        if (mProximitySensor != null) {
            if (DEBUG) {
                Slog.d(TAG, "mProximity = " + mProximity
                        ...
                        + ", mPowerRequest.useProximitySensor = " + mPowerRequest.useProximitySensor
                        + ", mPowerRequest.forceProximitySensorEnable = " + mPowerRequest.forceProximitySensorEnable
                        + ", mPowerRequest.forceWakeUpEnable = " + mPowerRequest.forceWakeUpEnable);
            }
            ...
            if (wantScreenOn(mPowerRequest.screenState)) {
                ...
                if (!mElectronBeamOffAnimator.isStarted()) {
                    ...
                    if (mPowerRequest.blockScreenOn
                        ...
                        blockScreenOn();
                        if (DEBUG) {
                            Slog.d(TAG, "mPowerState.updateElectronBeam");
                        }
                        ...
                        if (USE_ELECTRON_BEAM_ON_ANIMATION) {
                            if (!mElectronBeamOnAnimator.isStarted()) {
                                if (mPowerState.getElectronBeamLevel() == 1.0f) {
                                    ...
                                    Slog.d(TAG, "mElectronBeamOnAnimator.start");
                                    mElectronBeamOnAnimator.start();
                                } else {
                                    Slog.d(TAG, "mElectronBeamOnAnimator.end");
                                    mElectronBeamOnAnimator.end();
                                }
                            }
                        } else {
                            mPowerState.setElectronBeamLevel(1.0f);
                            Slog.d(TAG, "dismissElectronBeam");
                            mPowerState.dismissElectronBeam();
                            mSbScreenOnIsStart = false;
                        }
                    }
                ...
                if (!mElectronBeamOnAnimator.isStarted()) {
                    if (!mElectronBeamOffAnimator.isStarted()) {
                        if (mPowerState.getElectronBeamLevel() == 0.0f || mShutDownFlag_D) {
                            ...
                            Slog.d(TAG, "mElectronBeamOffAnimator.end");
    ...
    public interface Callbacks {
        ...
        void onSbProximityPositive();
        void onSbProximityNegative();
    }
    ...
    private final class DisplayControllerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SB_PROXIMITY_SENSOR_DEBOUNCED:
                    debounceSbProximitySensor();
                    break;
            }
        }
    ...
    private final SensorEventListener mLightSensorListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (DEBUG) {
                Slog.d(TAG, "mLightSensorListener.onSensorChanged(): enabled = " + mLightSensorEnabled +
                        ", lux = " + event.values[0]);
            }
