...
class WindowStateAnimator {
    ...
    boolean stepAnimationLocked(long currentTime) {
        ...
        Trace.traceBegin(Trace.TRACE_TAG_WINDOW_MANAGER | Trace.TRACE_TAG_PERF, "win animation done");
        if (WindowManagerService.DEBUG_ANIM) Slog.v(
    ...
    boolean finishDrawingLocked() {
        ...
        if (mDrawState == DRAW_PENDING) {
            ...
            if (!WindowManagerService.IS_USER_BUILD || WindowManagerService.DEBUG_SURFACE_TRACE || WindowManagerService.DEBUG_ANIM || WindowManagerService.SHOW_TRANSACTIONS || WindowManagerService.DEBUG_ORIENTATION)
    ...
    SurfaceControl createSurfaceLocked() {
        if (mSurfaceControl == null) {
            ...
            try {
                ...
                /// M: Enable log for focus window finding necessary conditions
                if (!WindowManagerService.IS_USER_BUILD || WindowManagerService.SHOW_TRANSACTIONS || WindowManagerService.SHOW_SURFACE_ALLOC) Slog.i(TAG,
                        "  CREATE SURFACE "
            ...
            try {
                try {
                    ...
                    /// M: When the surface is created, the value of flagsEx should be reset
                    /// as the init value like SF does. @{
                    if (FeatureOption.MTK_S3D_SUPPORT) {
                        mWin.mAttrs.flagsEx = WindowManager.LayoutParams.FLAG_EX_INIT_VALUE;
                    }
                    /// @}
                } catch (RuntimeException e) {
    ...
    void updateSurfaceWindowCrop(final boolean recoveringMemory) {
        ...
        /// @}
            ...
            applyDecorRect(new Rect(0, 0, displayInfo.logicalWidth, displayInfo.logicalHeight));
        } else {
        ...
        try {
            if (mSurfaceControl != null) {
                ...
                if (mWin.mTurnOnScreen) {
                    if (!WindowManagerService.IS_USER_BUILD || WindowManagerService.DEBUG_VISIBILITY) Slog.v(TAG,
                            "Show surface turning screen on: " + mWin);
                    mWin.mTurnOnScreen = false;
                    mAnimator.mBulkUpdateParams |= SET_TURN_ON_SCREEN;
                    /// M:[SmartBook] Record target window
                    if (FeatureOption.MTK_SMARTBOOK_SUPPORT) {
                        mService.mSbParams.setTurnOnScreen(mWin.mAttrs.getTitle().toString());
                        Slog.d(TAG, "Win:" + mService.mSbParams.mTurnOnScreenWin + " turning on screen");
                    }
