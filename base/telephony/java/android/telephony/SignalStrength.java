...
public class SignalStrength implements Parcelable {
    ...
    public int getGsmLevel() {
        ...
        try{
            ...
            if (mGsmRscpQdbm < 0) {
                // 3G network
                if (asu <= 2 || asu == 99) level = SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN;  //5
                else if (asu >= 12) level = SignalStrength.SIGNAL_STRENGTH_GREAT;    //15
                else if (asu >= 8)  level = SignalStrength.SIGNAL_STRENGTH_GOOD;				//12
                else if (asu >= 5/*9*/)  level = SignalStrength.SIGNAL_STRENGTH_MODERATE;
                else level = SignalStrength.SIGNAL_STRENGTH_POOR;
            // [ALPS01055164] -- END
            } else {
                // 2G network 
                if (asu <= 2 || asu == 99) level = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
                else if (asu >= 12) level = SIGNAL_STRENGTH_GREAT;	//12
                else if (asu >= 8)  level = SIGNAL_STRENGTH_GOOD;		//8
